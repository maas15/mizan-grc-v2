...
/* ══════════════════════════════════════════════════════════════════════════
  ...
  global.renderSectionFromJSON = function(json, sectionKey, isRtl, traceabilityRecord) {
    ...
    // Evidence level from traceability record
    var evidenceLevel = (traceabilityRecord && traceabilityRecord.evidence_sufficiency) || 'low';

    var title = section.number ? section.number + '. ' + (section.title || '') : (section.title || '');
    var traceIndicator = renderTraceabilityIndicator(sectionKey, evidenceLevel, isRtl);
    var html = title ? '<div class="section-banner">' + inlineHtml(title) + traceIndicator + '</div>' : '';
  ...
  global.renderStrategyFromJSON = function(json, isRtl, traceabilityRecords) {
    ...
    json.sections.forEach(function(section) {
      var trRec = null;
      if (traceabilityRecords && traceabilityRecords.length) {
        trRec = traceabilityRecords.find(function(r) {
          return r.section_key === section.key || r.section_key === 'full';
        });
      }
      var evidenceLevel = (trRec && trRec.evidence_sufficiency) || 'low';
      var title = section.number ? section.number + '. ' + (section.title || '') : (section.title || '');
      var traceIndicator = renderTraceabilityIndicator(section.key || section.number, evidenceLevel, isRtl);
      if (title) html += '<div class="section-banner">' + inlineHtml(title) + traceIndicator + '</div>';
      coalescePreviewTables(section.blocks || []).forEach(function(block) {
        if ((block.type || '').toLowerCase() === 'table') {
          html += renderTableWithQuality(block, isRtl);
        } else {
