  ...
  while(i < lines.length){
    ...
    if(s.startsWith('|')){
      ...
      function renderTable(tableLines, inheritedHeader){
        ...
        // Detect if this is a priority/likelihood/impact/status column by header text
        var prioColIdx = [];
        ...
        dataRows.forEach(function(row){
          ...
          row.forEach(function(c, ci){
            ...
            var isCriticalEmpty = isEmpty && (
              (isObjTable    && ci >= 2) ||   // Target Metric, Justification, Timeframe
              (isPillarTable && ci >= 2) ||   // Description, Expected Deliverable
              (isKpiTable    && ci >= 2) ||   // Target Value, Formula, Justification, Timeframe
              (isRiskTable   && ci >= 2)      // Likelihood, Impact, Mitigation Plan
            );
            if(isCriticalEmpty){
