  ...
  while(i < lines.length){
    ...
    if(s.startsWith('|')){
      ...
      function renderTable(tableLines, inheritedHeader){
        ...
        // ── Workstream RACI 7-col fix ───────────────────────────────────────
        // Header: Activity|Responsible|Accountable|Consulted|Informed|Timeline|Key Deliverable
        // AI often generates 6-cell data rows (omits Informed at index 4).
        // Insert '—' at position 4 so Timeline/Deliverable land in correct columns.
        var isRaci7 = (numCols === 7) && (function(){
          var h = header.map(function(c){ return c.toLowerCase(); });
        ...
        if(isRaci7){
          dataRows = dataRows.map(function(row){
            if(row.length === 6){
              var fixed = row.slice(0,4);
              fixed.push('—');
              fixed = fixed.concat(row.slice(4));
              return fixed;
            }
            return row;
          });
        ...
        // IMPORTANT: rows are already padded to numCols=5 before this runs,
        // so row.length===4 is never true. Instead detect by checking whether
        // the first cell is non-numeric — that means Action text slid into the
        // Step column. Shift everything right by 1, prepend counter.
        ...
        } else if(!isStepNumHeader && numCols > 2){
          // Non-# header (Activity, Initiative, Gap...): strip spurious row numbers
          // AI sometimes prepends a row number even when no # column exists
          dataRows = dataRows.map(function(row){
            var firstCell = (row[0]||'').trim();
