 ...
 *
 * Fixes two AI-generation defects in gap implementation guide tables:
 *
 * (a) MISSING DELIVERABLE COLUMN — The AI sometimes embeds the table header
 *     on the same line as the phase label and truncates the last column:
 *       "Immediate Actions : | Step | Action | Owner |"  (no Deliverable)
 *     After step E splits text from the pipe table we get a 3-column table.
 *     Fix: when a table in impl-guide context has header Step|Action|Owner
 *     (exactly 3 cols, no Deliverable), add Deliverable to header, separator,
 *     and data rows.
 *
 * (b) MISSING STEP NUMBERS / COLUMN SHIFT — The AI omits the step number
 *     for most rows, producing 3-cell data rows in a 4-column table.
 *     This shifts Action→Step, Owner→Action, Deliverable→Owner, and leaves
 *     the Deliverable column empty in every such row.
 *     Fix: for any 4-column impl-guide table data row with only 3 cells,
 *     prepend an empty step cell so columns align correctly.
 *
 * Must run AFTER step H (table block blank-line insertion) so that the
 * blank lines between phase sections are already in place.
 */
function fixImplGuideTables(md) {
    // Labels that introduce a gap implementation guide phase table
    var IMPL_LABEL_RE = /(Immediate Actions?|Short[-\s]?Term Goals?|Medium[-\s]?Term Goals?|Long[-\s]?Term Goals?|الإجراءات الفورية|أهداف.*قصيرة|أهداف.*متوسطة)/i;

    var lines     = md.split('\n');
    var out       = [];
    var inImplCtx = false;   // true after a phase label, until a new heading resets it
    var inTable   = false;   // true once we've seen the header row of the current table
    var sawSep    = false;   // true once we've seen the separator row
    var hdrCols   = 0;       // column count of the current table header

    for (var i = 0; i < lines.length; i++) {
        var line = lines[i];
        var t    = line.trim();

        // ── Detect phase label (sets impl context) ──────────────────────────
        if (!t.startsWith('|') && IMPL_LABEL_RE.test(t)) {
            inImplCtx = true;
            inTable   = false;
            sawSep    = false;
            hdrCols   = 0;
        ...
        // ── Blank line: reset table state (impl context persists) ────────────
        if (t === '') {
            inTable = false;
            sawSep  = false;
            hdrCols = 0;
        ...
        // ── Non-pipe, non-blank line: reset table state ──────────────────────
        if (!t.startsWith('|')) {
            inTable = false;
            sawSep  = false;
            hdrCols = 0;
        ...
        // ── Pipe row outside impl context: pass through untouched ───────────
        if (!inImplCtx) {
            out.push(line);
            continue;
        }
        ...
        // ── Header row (first pipe row in this context) ──────────────────────
        if (!inTable) {
            inTable = true;

            if (!isSep) {
                // Real header row — check if Deliverable column is missing
                ...
                if (hasStep && hasAction && hasOwner && !hasDeliv && cells.length === 3) {
        ...
        if (isSep) {
            sawSep = true;
            if (hdrCols > 0 && cells.length !== hdrCols) {
        ...
        // If the table has 4 columns but this row only has 3, the step number
        // was omitted — prepend an empty step cell to realign columns.
        if (sawSep && hdrCols === 4 && cells.length === 3) {
            line = '|  | ' + cells.join(' | ') + ' |';
        }
 ...
 *  L.  Fix gap impl-guide tables: add missing Deliverable column + empty step cells
...
function preprocessGRCMarkdown(md) {
    ...
    // Must run after H so that blank-line boundaries between phase sections
    // are already in place (the state machine relies on blank lines to reset).
