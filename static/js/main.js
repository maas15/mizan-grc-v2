...
function toggleTheme() {
    ...
    // Set color-scheme on <html> for native controls (scrollbars, selects, etc.)
    ...
    // ── Sidebar sync: Force sidebar repaint so gradient updates instantly ──
    document.querySelectorAll('.sidebar, .sidebar-left, .sidebar-right').forEach(function (el) {
        el.style.transition = 'background 0.3s ease, border-color 0.3s ease, color 0.3s ease';
    });
    ...
    setTimeout(function () {
        ...
        // Clean up inline transitions after animation finishes
        document.querySelectorAll('.sidebar, .sidebar-left, .sidebar-right').forEach(function (el) {
            el.style.transition = '';
        });
    }, 400);
...
const MizanPipe = {
     ...
     * @param {Object} pipeResult - { risks_added: N, initiatives_added: N }
     */
    notify(pipeResult) {
        ...
        if (pipeResult.initiatives_added > 0) {
            parts.push(pipeResult.initiatives_added + ' initiative(s) \u2192 Project Tasks');
        }
        if (parts.length > 0) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ensure color-scheme is set on load for native controls
