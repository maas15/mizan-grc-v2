 ...
 *  4. Split AI-merged heading+sublabel:  "## 1. Title Sublabel:"  →  two lines
 *  5. Repair separator rows that AI concatenated with the next data row
 *  6. Collapse 4+ consecutive newlines to 3
 *
...
function preprocessGRCMarkdown(md) {
    ...
    // ── 4. Split merged heading + sublabel ────────────────────────────────
    // AI sometimes writes:  "## 1. Vision & Objectives Vision:\n"
    // We want:              "## 1. Vision & Objectives\n\n**Vision:**\n"
    //
    // Pattern: ## N. <title words> <SomeSublabel:>
    //   - heading marker  : #{2,4}
    //   - optional number : \d+\.\s+
    //   - title text      : [^\n:]{5,45}?  (non-greedy, no colon, 5-45 chars)
    //   - whitespace gap  : \s+
    //   - sublabel        : [A-Z][a-zA-Z &\/()]{2,30}:  (Capital-led, ends with :)
    md = md.replace(
        /(#{2,4}\s+(?:\d+\.\s+)?[^\n:]{5,45}?)\s+([A-Z][a-zA-Z &\/()]{2,30}:)([ \t]*)(\n)/g,
        function (match, heading, label, _sp, nl) {
            // Safety: don't split if the heading is already very short
            // (avoids false positives on "## Vision:" style headings)
    ...
    // ── 5. Repair separator rows concatenated with a data row ─────────────
    // AI sometimes writes:  |---|---|---| | 1 | Content
    // We want two lines:    |---|---|---|
    //                       | 1 | Content
    md = md.replace(
        /(\|\s*[-:]+\s*(?:\|\s*[-:]+\s*)+\|)([ \t]*)(\|\s*\S)/g,
    ...
    // ── 6. Collapse excessive blank lines (>3 consecutive newlines → 3) ───
    md = md.replace(/\n{4,}/g, '\n\n\n');
