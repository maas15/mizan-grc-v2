...
// ============================================================================
// PILLAR API HELPERS (used by dashboard and domain pages)
// ============================================================================

const MizanAPI = {
    // Pillar 2.2: Compliance Snapshot
    async takeComplianceSnapshot() {
        try {
            const res = await API.fetch('/api/compliance/snapshot', { method: 'POST' });
            if (res.success) {
    ...
    async getComplianceTrend(days = 90) {
        try {
            return await API.fetch(`/api/compliance/trend?days=${days}`);
        } catch (e) { return null; }
    },

    // Pillar 3.1: Token Usage
    async getTokenUsage() {
        try {
            return await API.fetch('/api/user/token-usage');
        } catch (e) { return null; }
    },

    // Pillar 3.2: Dynamic Frameworks
    async getFrameworks(domain = '') {
        try {
            const url = domain ? `/api/frameworks?domain=${encodeURIComponent(domain)}` : '/api/frameworks';
            return await API.fetch(url);
        } catch (e) { return null; }
    },

    // Pillar 4.2: Logo Upload
    ...
    async deleteLogo() {
        try {
            return await API.fetch('/api/user/logo', { method: 'DELETE' });
        } catch (e) { return null; }
    }
};

window.MizanAPI = MizanAPI;
