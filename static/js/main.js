 ...
 *  NEW-1. Split heading (any level) from inline pipe table on same line
 *  NEW-2a. Split metadata rows packed on one line (| | **Key** | Value | | **Key** |...)
 *  NEW-2b. Split two numbered data rows packed on same line (| 1 |...| | 2 |...)
 *  NEW-3. Remove repeat separator rows that follow numbered data rows
 *  NEW-4. Trim leading whitespace from rows created by splitting
 *  2. Ensure blank line BEFORE every table row  (|...)
 *  3. Ensure blank line AFTER the last row of every table block
 *  4a. Split AI-merged heading+sublabel where body text follows on same line
 *  4b. Split AI-merged heading+sublabel where sublabel is at end of line
 *  5. Reconnect split table blocks (blank+orphan-separator → removed)
 *  6. Repair separator rows concatenated with next data row on same line
 *  7. Collapse 4+ consecutive newlines to 3
 *
...
function preprocessGRCMarkdown(md) {
    ...
    // ── NEW-1. Split heading from inline pipe table on same line ─────────────
    // AI sometimes writes: "# Cyber Security Strategy | | | | **Key** | Value |..."
    // The heading and the metadata table are on the same physical line.
    // Split at the point where heading text ends and the first pipe pair starts.
    md = md.replace(/(#{1,4}[^|\n]+?)\s*(\|\s*[|\s]*\|)/g, '$1\n\n$2');

    // ── NEW-2a. Split metadata rows packed on one line ────────────────────
    // AI outputs: "| | | | **Document Type** | Value | | **Domain** | Value |"
    // Split at each "| | **Capital" boundary so each key-value pair is on its own line.
    md = md.replace(/(\|)(\s*\|\s*\*\*[A-Z\u0600-\u06FF])/g, '$1\n$2');

    // ── NEW-2b. Split two numbered data rows packed on same line ─────────
    // AI outputs: "| 1 | content | | 2 | content |" — two rows on one line.
    // Split at "| | NUMBER" where a new numbered row begins.
    md = md.replace(/(\|)(\s*\|\s*\d+\s*\|)/g, '$1\n$2');

    // ── NEW-3. Remove repeat separator rows after numbered data rows ──────
    // After splitting double rows, the AI's repeat separators become orphans:
    //   | 2 | content |          ← data row
    //   |---|---|---|             ← orphan separator (no header → rows below break)
    // Remove any separator row that immediately follows a numbered data row.
    md = md.replace(/(^[ \t]*\|[ \t]*\d+[ \t]*\|[^\n]+\n)([ \t]*\|[-: |]+\|[ \t]*\n)/gm, '$1');

    // ── NEW-4. Trim leading whitespace from rows introduced by row-splitting ──
    md = md.replace(/^[ \t]+(\|)/gm, '$1');

    // ── 2. Ensure blank line before any table row ──────────────────────────
    // Match: a non-newline character, followed by a newline, followed by |
    // Insert an extra newline so marked sees a paragraph break before the table.
    md = md.replace(/([^\n])\n(\|)/g, '$1\n\n$2');

    // ── 3. Ensure blank line after the last row of a table block ──────────
    // Match: a | row immediately followed by a non-blank, non-| next line.
    // Insert a blank line so marked knows the table has ended.
    md = md.replace(/(\|[^\n]*\n)(?!\n)(?!\|)([^\n])/g, '$1\n$2');

    // ── 4a. Split merged heading + sublabel + BODY TEXT (Case A) ────────────
    // AI sometimes writes the entire body paragraph on the heading line:
    //   "## 1. Vision & Objectives Vision: The Organization aspires to become..."
    // The 30-char minimum on the body text distinguishes this from a normal
    // heading that simply ends with a colon-word (e.g. "## 3. Compliance Overview:").
    md = md.replace(
        /(#{2,4}\s+(?:\d+\.\s+)?[^\n]+?)\s+([A-Z][a-zA-Z]{2,20}:)\s+([^\n]{30,})/g,
    ...
    // ── 4b. Split merged heading + sublabel at LINE END (Case B) ─────────
    // AI sometimes writes:  "## 1. Vision & Objectives Vision:\n"
    // We want:              "## 1. Vision & Objectives\n\n**Vision:**\n"
    md = md.replace(
        /(#{2,4}\s+(?:\d+\.\s+)?[^\n]+?)\s+([A-Z][a-zA-Z]{2,20}:)([ \t]*)(?=\n)/g,
    ...
    // ── 5. Reconnect split table blocks (orphan separator after blank line) ──
    // AI sometimes breaks a single table into two blocks:
    //   | row 2 |          ← end of first block (blank line terminates table)
    //                      ← blank line
    //   |---|---|           ← orphan separator — has no header, so rows below
    //   | row 3 |            render as raw pipe text instead of table cells
    //
    // Fix: remove the blank + orphan separator to merge the blocks back together.
    // The (?=\|) lookahead ensures we only remove separators that have data rows
    // below them (i.e. they really are a continuation, not just trailing noise).
    md = md.replace(
        /(\|[^\n]+\n)\n(\|\s*[-:]+\s*(?:\|\s*[-:]+\s*)+\|\s*\n)(?=\|)/g,
        '$1'
    );

    // ── 6. Repair separator rows concatenated with a data row ─────────────
    // AI sometimes writes:  |---|---|---| | 1 | Content  (on same line)
    // We want two lines:    |---|---|---|
    //                       | 1 | Content
    md = md.replace(
        /(\|\s*[-:]+\s*(?:\|\s*[-:]+\s*)+\|)([ \t]*)(\|\s*\S)/g,
    ...
    // ── 7. Collapse excessive blank lines (>3 consecutive newlines → 3) ───
    md = md.replace(/\n{4,}/g, '\n\n\n');
