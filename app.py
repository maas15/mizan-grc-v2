        ...
        def process_arabic(text, font_name_for_wrap=None, font_size_for_wrap=11, extra_indent=0):
            ...
            if is_arabic and text:
                try:
                    ...
                    # Allow ~8% overflow to prevent orphaning words at line boundaries
                    overflow_tolerance = wrap_width * 0.08
                    ...
                    for word in words:
                        ...
                        if test_width > wrap_width and current_line_words:
                            # Check if the overflow is within tolerance — keep word on this line
                            if test_width <= wrap_width + overflow_tolerance:
                                current_line_words.append(word)
                                current_width = test_width
                                continue
                            # This line is full — wrap to next line
                    ...
                    # Step 3b: Orphan/widow protection
                    # If any line has only 1 word and there's a previous line,
                    # pull the last word from previous line to join it
                    if len(lines) > 1:
                        fixed_lines = [lines[0]]
                        for i in range(1, len(lines)):
                            # Check if this line is very short (orphan)
                            line_w = stringWidth(lines[i], wrap_font, font_size_for_wrap)
                            if line_w < wrap_width * 0.15 and i > 0:
                                # This is an orphan — rebuild previous + current from their words
                                # We need to undo get_display to get back to reshaped words
                                # Simpler: re-wrap the last 2 lines together with a slightly wider limit
                                prev_display = fixed_lines[-1]
                                curr_display = lines[i]
                                # Combine them with wider tolerance
                                combined = prev_display + ' ' + curr_display
                                combined_w = stringWidth(combined, wrap_font, font_size_for_wrap)
                                if combined_w <= wrap_width + overflow_tolerance * 2:
                                    # They fit together — merge
                ...
                try:
                    ...
                    for word in words:
                        ...
                        if test_width > cell_width and current_line_words:
                            if test_width <= cell_width + overflow_t:
                                current_line_words.append(word)
                                current_width = test_width
                                continue
                            line_text = ' '.join(current_line_words)
                    ...
                    # Orphan protection for table cells too
                    if len(lines) > 1:
                        fixed = [lines[0]]
                        for i in range(1, len(lines)):
                            line_w = stringWidth(lines[i], arabic_font_name, font_size)
                            if line_w < cell_width * 0.15 and i > 0:
                                combined = fixed[-1] + ' ' + lines[i]
                                combined_w = stringWidth(combined, arabic_font_name, font_size)
                                if combined_w <= cell_width + overflow_t * 2:
                                    fixed[-1] = combined
                                else:
