    ...
    try:
        ...
        if lang == 'ar':
            ...
            for i, (gap_name, gap_desc) in enumerate(gaps, 1):
                ...
                gaps_table += f"| {i} | {gap_name} | {gap_desc} | {priority} | يحتاج تقييم |\n"
            
            prompt = f"""أنشئ وثيقة استراتيجية احترافية لمجال **{domain_desc}** وفق إطار {frameworks_list}.

المنظمة: {data.get('org_name', 'المنظمة')}
القطاع: {data.get('sector', 'حكومي')}

⚠️ قواعد صارمة:
- المحتوى يجب أن يكون خاصاً بمجال **{domain}** فقط
- لا تستخدم محتوى من مجالات أخرى
- لا تستخدم أي نسب مئوية محددة
...
**الرؤية الاستراتيجية:**
(اكتب رؤية خاصة بمجال {domain} تناسب المنظمة)

### الأهداف الاستراتيجية:
(اكتب 5 أهداف استراتيجية خاصة بمجال {domain})
...
|---|--------|-------|---------|--------|
{gaps_table}
(اكتب أدلة تنفيذ لكل فجوة - 5 خطوات لكل فجوة)

[SECTION]

## 3. الركائز الاستراتيجية
(اكتب 4 ركائز استراتيجية خاصة بمجال {domain})

[SECTION]

## 4. خارطة الطريق
(اكتب 3 مراحل - كل مرحلة 4 أنشطة خاصة بمجال {domain})

[SECTION]

## 5. مؤشرات الأداء الرئيسية
(اكتب 8 مؤشرات KPI خاصة بمجال {domain})

[SECTION]

## 6. تقييم الجاهزية والمخاطر
(اكتب 5 عوامل نجاح و4 مخاطر خاصة بمجال {domain})
