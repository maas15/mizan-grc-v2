...
class Config:
    ...
    AI_PROVIDER = os.getenv('AI_PROVIDER', 'auto')  # auto, openai, anthropic, google
...
def check_ai_available():
    ...
    return bool(config.OPENAI_API_KEY or config.ANTHROPIC_API_KEY or config.GOOGLE_API_KEY)
...
def get_ai_status():
    ...
    names = {'openai': 'OpenAI', 'anthropic': 'Anthropic Claude', 'google': 'Google Gemini'}
    ...
    try:
        ...
        for fallback in ['anthropic', 'openai', 'google']:
            if fallback == provider:
                continue
            key_map = {'anthropic': config.ANTHROPIC_API_KEY, 'openai': config.OPENAI_API_KEY, 'google': config.GOOGLE_API_KEY}
