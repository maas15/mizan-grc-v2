...
def api_podcast_generate():
    ...
    try:
        ...
        if lang == 'ar':
            ...
            prompt = f"""أنت كاتب سيناريو حوار تلفزيوني محترف. اكتب حواراً طبيعياً وحيوياً بالعربية الفصحى بين خبيرين:
- م. نورة (المتحدث A): مديرة امتثال بخبرة 10 سنوات، أسلوبها عملي ومباشر
- د. خالد (المتحدث B): مستشار حوكمة ومخاطر وامتثال أول بخبرة 15 سنة، أسلوبه تحليلي واستراتيجي

الموضوع: {topic_name} في مجال {domain}
الأطر المرجعية: {fw_text_ar}

أجب بصيغة JSON فقط بدون أي نص إضافي أو backticks:
{{"dialogue": [
  {{"speaker": "A", "text": "مرحباً بكم في نقاش ميزان..."}},
  {{"speaker": "B", "text": "أهلاً نورة..."}},
  ... (18-22 سطر حوار متنوع بين A وB)
]}}

قواعد مهمة جداً:
- اكتب الحوار بالكامل باللغة العربية الفصحى الواضحة
- اكتب أسماء الأطر والمعايير بالعربية وليس بالإنجليزية (مثلاً: الضوابط الأساسية للأمن السيبراني وليس NCA ECC)
- اكتب الأرقام بالحروف العربية عند الإمكان (مثلاً: خمسة مجالات وليس 5 مجالات)
- لا تستخدم أي اختصارات إنجليزية — اكتب الاسم العربي الكامل
- الحوار يجب أن يبدأ بتقديم الموضوع وينتهي بخلاصة عملية
- اذكر أمثلة حقيقية من بيئة العمل السعودية والخليجية
- اجعل الحوار يتضمن نقاشاً وتبادل آراء وليس مجرد سرد
- كل سطر جملتان إلى أربع جمل — مناسب للقراءة الصوتية الطبيعية
- لا تستخدم رموز تعبيرية أو نجوم أو أقواس أو علامات ترقيم غير عربية"""
        else:
            prompt = f"""You are a professional discussion script writer. Write a natural, engaging dialogue between two experts:
- Dr. Sarah (Speaker A): Senior GRC Strategist with 15 years experience, analytical and strategic style, female
- James (Speaker B): Compliance Director with 10 years experience, practical and direct style, male

Topic: {topic_name} in {domain}
Reference frameworks: {fw_text}

Respond with JSON ONLY, no extra text or backticks:
{{"dialogue": [
  {{"speaker": "A", "text": "Welcome to the Mizan GRC discussion..."}},
  {{"speaker": "B", "text": "Thanks Sarah..."}},
  ... (18-22 dialogue lines alternating between A and B)
]}}

Important rules:
- Start with topic introduction, end with practical takeaways
- Reference specific frameworks and standards from the list above
- Include real-world examples, debates, and differing perspectives
- Each line should be 2-4 sentences — suitable for natural voice reading
- Make it sound like a real professional conversation, not a lecture
