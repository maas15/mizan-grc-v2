...
# ============================================================================
# Render has a 30s request timeout, so we run AI generation in background threads
# and let the client poll for results.
background_tasks = {}  # task_id -> {'status': 'pending'|'done'|'error', 'result': ..., 'error': ...}
...
def cleanup_old_tasks():
    ...
    with background_tasks_lock:
        to_remove = [tid for tid, t in background_tasks.items() if t.get('created', datetime.now()) < cutoff]
        for tid in to_remove:
            del background_tasks[tid]

def run_ai_task(task_id, prompt, lang, callback_data=None):
...
Write the complete modified policy (not just the changes):"""
        ...
        t = threading.Thread(target=run_ai_task, args=(task_id, prompt, lang, callback_data))
        t.daemon = True
        t.start()
    ...
    try:
        ...
        def process_arabic(text):
            ...
            if is_arabic and text:
                try:
                    # Clean text first - remove any problematic characters
                    text = str(text).strip()
                    if not text:
                        return text
                    
                    # Handle mixed content: if text has HTML tags like <b>, process parts separately
