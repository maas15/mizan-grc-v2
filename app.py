...
def ensure_markdown_formatting(text):
    ...
    - Table rows merged onto heading lines (most critical fix)
    """
    import re
    if not text:
        return text
    
    # 0. CRITICAL: Split heading + table merged on same line
    # e.g., "### Strategic Objectives: | # | Objective |..." → heading then table on next line
    # Also handles "## 1. Vision & Objectives ### Strategic Objectives: | # | Obj..."
    text = re.sub(r'(#{2,4}\s+[^|\n]+?:?)\s*(\|\s*#?\s*\|)', r'\1\n\n\2', text)
    ...
    # e.g., "| 1 | text | | 2 | text |" → split into separate rows
    lines = text.split('\n')
    fixed_lines = []
    for line in lines:
        stripped = line.strip()
        # Check if line has multiple table rows merged (more than 2 rows worth of | characters)
        if stripped.startswith('|') and stripped.count('|') > 10:
            # Try to split by detecting "| |" patterns that indicate row boundaries
            # A row boundary looks like: "| \n| " or "| | 1 |" where a cell ends and a new # starts
    ...
    # 12. Fix consecutive blank lines (max 2)
    text = re.sub(r'\n{4,}', '\n\n\n', text)
...
**Owner:** [Responsible Department]"""
        ...
        if is_procedure:
            ...
            prompt += f"""\n\nIMPORTANT: This is a PROCEDURE document for "{fw_name}", not a policy. Generate a detailed step-by-step operational procedure that directly addresses the mandatory controls and requirements of {fw_name}.

Structure the procedure with:
1. Purpose and scope (reference specific {fw_name} control requirements)
2. Applicable {fw_name} controls/clauses this procedure addresses
3. Roles and responsibilities (RACI matrix)
4. Prerequisites and required tools
5. Detailed step-by-step instructions with numbered steps (each step must reference the specific {fw_name} requirement it fulfills)
6. Decision flowchart points (if/then conditions)
7. Verification and validation steps
8. Exception handling procedures
9. Evidence and documentation requirements for {fw_name} compliance audits
10. Forms/templates/checklists to use
11. Record keeping requirements
12. Review and update frequency
Format all steps as numbered lists with clear action verbs. Include tables for RACI, tools, and checklists. Reference specific {fw_name} control IDs/clauses where applicable."""
        content = generate_ai_content(prompt, lang, content_type='policy')
