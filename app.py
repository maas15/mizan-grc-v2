...
# If SECRET_KEY env var is missing, generate ONCE and persist to .mizan_secret
# so restarts don't invalidate every user session.
_secret = os.getenv('SECRET_KEY')
if not _secret:
    _secret_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.mizan_secret')
    ...
    if not _secret:
        ...
        try:
            with open(_secret_file, 'w') as f:
                f.write(_secret)
            print("INFO: Generated persistent SECRET_KEY → .mizan_secret", flush=True)
        except OSError:
            print("WARN: Could not persist SECRET_KEY to disk", flush=True)
...
def get_db():
    ...
    if '_database' not in g:
        g._database = sqlite3.connect(config.DB_PATH, timeout=10)
...
def init_db():
    ...
    # ── NEW COLUMNS: Mizan Pipe — Unified Risk Register domain tracking ──
    try:
    ...
    # ── NEW COLUMNS: Mizan Pipe — Project task source tracking ──
    try:
...
# ============================================================================
# MIZAN PIPE — Automatic Risk-to-Register & Initiative-to-Project Parsers
# ============================================================================

def pipe_risks_to_register(content, user_id, domain='General'):
    """Parse 'Key Risks' table from AI output and auto-INSERT into risk_register.
    
    Scans for Markdown tables with risk/likelihood/impact columns and inserts
    each row as a new risk entry linked to the originating domain.
    Returns list of inserted risk_ids.
    """
    import re
    inserted = []
    
    # Match table rows: | N | Risk Name | Likelihood | Impact | Mitigation |
    # Supports both English and Arabic table formats
    risk_table_pattern = r'\|\s*\d+\s*\|\s*([^|]+?)\s*\|\s*(High|Medium|Low|عالية?|متوسطة?|منخفضة?)\s*\|\s*(High|Medium|Low|عالي|متوسط|منخفض)\s*\|\s*([^|]+?)\s*\|'
    
    matches = re.findall(risk_table_pattern, content, re.IGNORECASE)
    if not matches:
        return inserted
    
    # Map text to numeric likelihood/impact
    score_map = {
        'high': 4, 'عالية': 4, 'عالي': 4,
        'medium': 3, 'متوسطة': 3, 'متوسط': 3,
        'low': 2, 'منخفضة': 2, 'منخفض': 2,
    }
    
    try:
        conn = get_db()
        for risk_name, likelihood_text, impact_text, mitigation in matches:
            risk_name = risk_name.strip()
            mitigation = mitigation.strip()
            
            # Skip header-like rows
            if risk_name.lower() in ('risk', 'الخطر', 'risk name', 'threat', 'التهديد'):
                continue
            
            likelihood = score_map.get(likelihood_text.lower().strip(), 3)
            impact = score_map.get(impact_text.lower().strip(), 3)
            
            conn.execute('''
                INSERT INTO risk_register 
                (user_id, name, description, category, likelihood, impact, 
                 treatment, treatment_plan, status, domain, source_type)
                VALUES (?, ?, ?, ?, ?, ?, 'Mitigate', ?, 'Open', ?, 'ai_strategy')
            ''', (user_id, risk_name[:200], f"Auto-extracted from {domain} strategy generation",
                  'Strategic', likelihood, impact, mitigation[:500], domain))
            
            risk_id = conn.execute('SELECT last_insert_rowid()').fetchone()[0]
            inserted.append(risk_id)
            
            # Also auto-create mitigation task for high-score risks
            risk_score = likelihood * impact
            auto_create_mitigation_task(user_id, risk_id, risk_name, risk_score, domain)
        
        if inserted:
            conn.commit()
            log_action(user_id, 'pipe_risks_to_register', {
                'domain': domain, 'count': len(inserted), 'risk_ids': inserted
            })
            print(f"✅ MIZAN PIPE: Inserted {len(inserted)} risks into register from {domain} strategy", flush=True)
    except Exception as e:
        print(f"Mizan Pipe risk-to-register error (non-fatal): {e}", flush=True)
    
    return inserted


def pipe_initiatives_to_projects(content, user_id, domain='General'):
    """Parse 'Strategic Pillars' initiatives from AI output and auto-INSERT into project_tasks.
    
    Scans for bullet-point initiatives (• Initiative: ...) and table rows
    from implementation roadmaps.
    Returns list of inserted task_ids.
    """
    import re
    inserted = []
    
    # Pattern 1: • Initiative: [text]
    initiative_pattern = r'[•●]\s*Initiative:\s*(.+?)(?:\n|$)'
    matches = re.findall(initiative_pattern, content)
    
    # Pattern 2: Arabic bullet initiatives
    ar_pattern = r'[•●]\s*(?:المبادرة|مبادرة):\s*(.+?)(?:\n|$)'
    ...
    try:
        conn = get_db()
        for initiative_text in matches:
            title = initiative_text.strip()[:200]
            ...
            conn.execute('''
                ...
                VALUES (?, ?, ?, ?, 'todo', 'medium', 'strategic_initiative', 'ai_strategy')
            ''', (user_id, domain, title,
                  f"Auto-extracted strategic initiative from {domain} strategy generation"))
        ...
        if inserted:
            conn.commit()
            log_action(user_id, 'pipe_initiatives_to_projects', {
                'domain': domain, 'count': len(inserted), 'task_ids': inserted
            })
            print(f"✅ MIZAN PIPE: Inserted {len(inserted)} initiatives into projects from {domain} strategy", flush=True)
    except Exception as e:
        print(f"Mizan Pipe initiative-to-project error (non-fatal): {e}", flush=True)
    
    return inserted


def pipe_risk_assessment_to_register(content, user_id, domain='General', asset='', threat=''):
    """Parse Risk Assessment output and auto-INSERT the identified risk into risk_register.
    
    Extracts: risk level, risk score from the Risk Assessment Summary table,
    and creates a risk_register entry.
    Returns list of inserted risk_ids.
    """
    import re
    inserted = []
    
    # Extract risk level from summary table
    ...
    level_to_scores = {
        'critical': (5, 5), 'حرج': (5, 5),
        'high': (4, 4), 'عالي': (4, 4),
        'medium': (3, 3), 'متوسط': (3, 3),
        'low': (2, 2), 'منخفض': (2, 2),
    }
    likelihood, impact = level_to_scores.get(level_text, (3, 3))
    
    # If we have a numeric score, refine the mapping
    if score_val >= 9:
    ...
    # Build risk name from threat + asset
    risk_name = threat[:150] if threat else 'Risk from assessment'
    if asset:
        risk_name = f"{threat[:100]} → {asset[:50]}" if threat else f"Risk to {asset[:100]}"
    
    # Extract category from summary table
    cat_match = re.search(
        r'(?:Risk Category|فئة الخطر)\s*\|\s*([^|\n]+)',
        content
    )
    category = cat_match.group(1).strip()[:50] if cat_match else 'Operational'
    ...
    try:
        ...
        ''', (user_id, risk_name[:200],
              f"Auto-extracted from {domain} risk assessment",
              category, likelihood, impact, domain))
        
        risk_id = conn.execute('SELECT last_insert_rowid()').fetchone()[0]
        inserted.append(risk_id)
        
        # Also extract recommended controls as sub-risks or mitigation tasks
        control_pattern = r'\|\s*\d+\s*\|\s*([^|]+?)\s*\|\s*(High|Medium|Low|عالية?|متوسطة?|منخفضة?)\s*\|'
        controls = re.findall(control_pattern, content, re.IGNORECASE)
        
        for control_name, priority_text in controls:
            control_name = control_name.strip()
            if not control_name or control_name.lower() in ('control', 'الضابط', 'control name', '#', 'اسم الضابط'):
                continue
            
            # Create mitigation task for each control
            ctrl_priority = 'high' if priority_text.lower() in ('high', 'عالية') else ('medium' if priority_text.lower() in ('medium', 'متوسطة') else 'low')
            conn.execute('''
                INSERT INTO project_tasks 
                (user_id, domain, title, description, status, priority, category, source_type, mitigation_for_risk_id)
                VALUES (?, ?, ?, ?, 'todo', ?, 'gap_mitigation', 'ai_risk_assessment', ?)
            ''', (user_id, domain, f"Implement: {control_name[:150]}",
                  f"Control from {domain} risk assessment for: {risk_name[:100]}",
                  ctrl_priority, risk_id))
        
        conn.commit()
        log_action(user_id, 'pipe_risk_assessment_to_register', {
            'domain': domain, 'count': len(inserted), 'risk_ids': inserted,
            'controls_extracted': len(controls)
        })
        print(f"✅ MIZAN PIPE: Inserted {len(inserted)} risks + {len(controls)} controls from {domain} risk assessment", flush=True)
    except Exception as e:
        print(f"Mizan Pipe risk-assessment-to-register error (non-fatal): {e}", flush=True)
    
    return inserted


def pipe_kpis_to_tasks(content, user_id, domain='General'):
    """Parse KPI table from strategy output and auto-INSERT into project_tasks.
    
    Extracts: KPI name, description, target value, timeframe from the KPI table.
    Returns list of inserted task_ids.
    """
    import re
    inserted = []
    
    # Match KPI table rows: | N | KPI Name | Description | ... | Timeframe | ...
    # English: | 1 | KPI Name | Description | Current | Target | Justification | Timeframe | Source |
    kpi_pattern = r'\|\s*\d+\s*\|\s*([^|]{5,}?)\s*\|\s*([^|]{5,}?)\s*\|(?:[^|]*\|){1,4}\s*(?:Within\s+\d+\s+months?|خلال\s+\d+\s+(?:أشهر|شهر))[^|]*\|'
    matches = re.findall(kpi_pattern, content, re.IGNORECASE)
    
    # Also try simpler pattern
    if not matches:
        kpi_pattern2 = r'\|\s*\d+\s*\|\s*([^|]{8,80}?)\s*\|\s*([^|]{8,}?)\s*\|'
        kpi_section = ''
        # Find the KPI section
        kpi_start = content.find('## 5.') if '## 5.' in content else content.find('## ٥.')
        if kpi_start == -1:
            kpi_start = content.find('Key Performance Indicators') if 'Key Performance Indicators' in content else content.find('مؤشرات الأداء')
        if kpi_start >= 0:
            kpi_end = content.find('[SECTION]', kpi_start)
            if kpi_end == -1:
                kpi_end = content.find('## 6.', kpi_start)
            if kpi_end == -1:
                kpi_end = kpi_start + 5000
            kpi_section = content[kpi_start:kpi_end]
            matches = re.findall(kpi_pattern2, kpi_section)
    ...
    try:
        conn = get_db()
        for kpi_name, kpi_desc in matches:
            title = kpi_name.strip()[:200]
            desc = kpi_desc.strip()[:500]
            
            if not title or len(title) < 5:
                continue
            # Skip header-like rows
            if title.lower() in ('kpi', 'kpi name', 'المؤشر', 'اسم المؤشر', '#', 'description'):
                continue
            # Skip placeholders
            if title.startswith('[') and title.endswith(']'):
                continue
            ...
            ''', (user_id, domain, f"KPI: {title}",
                  f"{desc}\n\nAuto-extracted from {domain} strategy KPIs"))
        ...
        if inserted:
            conn.commit()
            log_action(user_id, 'pipe_kpis_to_tasks', {
                'domain': domain, 'count': len(inserted)
            })
            print(f"✅ MIZAN PIPE: Inserted {len(inserted)} KPIs from {domain} strategy", flush=True)
    except Exception as e:
        print(f"Mizan Pipe KPI extraction error (non-fatal): {e}", flush=True)
    
    return inserted


def pipe_gaps_to_tasks(content, user_id, domain='General'):
    """Parse Gap Analysis table from strategy output and auto-INSERT into project_tasks.
    
    Extracts: gap name, description, priority from the gaps table.
    Returns list of inserted task_ids.
    """
    import re
    inserted = []
    
    # Match gap table rows: | N | Gap Name | Description | Priority | Status |
    # Also supports multi-reg with Framework column
    gap_pattern = r'\|\s*\d+\s*\|\s*([^|]{5,80}?)\s*\|\s*([^|]{5,}?)\s*\|\s*(?:[^|]*\|\s*)?(Critical|High|Medium|Low|حرج|عالي|عالية|متوسط|متوسطة|منخفض|منخفضة)\s*\|'
    ...
    priority_map = {
        ...
        'high': 'high', 'عالي': 'high', 'عالية': 'high',
    ...
    try:
        conn = get_db()
        for gap_name, gap_desc, priority_text in matches:
            title = gap_name.strip()[:200]
            desc = gap_desc.strip()[:500]
            
            if not title or len(title) < 5:
                continue
            # Skip header-like rows
            if title.lower() in ('gap', 'الفجوة', 'gap name', '#', 'description'):
                continue
            if title.startswith('[') and title.endswith(']'):
                continue
            
            priority = priority_map.get(priority_text.strip().lower(), 'medium')
            
            conn.execute('''
                INSERT INTO project_tasks 
                (user_id, domain, title, description, status, priority, category, source_type)
                VALUES (?, ?, ?, ?, 'todo', ?, 'gap_mitigation', 'ai_strategy')
            ''', (user_id, domain, f"Close Gap: {title}",
                  f"{desc}\n\nAuto-extracted from {domain} strategy gap analysis",
                  priority))
        ...
        if inserted:
            conn.commit()
            log_action(user_id, 'pipe_gaps_to_tasks', {
                'domain': domain, 'count': len(inserted)
            })
            print(f"✅ MIZAN PIPE: Inserted {len(inserted)} gap mitigation tasks from {domain} strategy", flush=True)
    except Exception as e:
        print(f"Mizan Pipe gap extraction error (non-fatal): {e}", flush=True)
    
    return inserted


def pipe_roadmap_to_tasks(content, user_id, domain='General'):
    """Parse Implementation Roadmap table from strategy output and auto-INSERT into project_tasks.
    
    Extracts: activity name, owner, timeline, deliverable from each phase.
    Returns list of inserted task_ids.
    """
    import re
    inserted = []
    
    # Match roadmap table rows: | N | Activity | Owner | Timeline | Deliverable |
    roadmap_pattern = r'\|\s*\d+\s*\|\s*([^|]{5,100}?)\s*\|\s*([^|]{2,60}?)\s*\|\s*(Month\s+\d+[\-–]\d+|خلال[^|]*|الشهر[^|]*|[^|]{3,40}?)\s*\|\s*([^|]{3,}?)\s*\|'
    
    # Find the roadmap section
    roadmap_start = content.find('## 4.') if '## 4.' in content else content.find('## ٤.')
    if roadmap_start == -1:
        roadmap_start = content.find('Implementation Roadmap') if 'Implementation Roadmap' in content else content.find('خارطة الطريق')
    if roadmap_start == -1:
        return inserted
    
    roadmap_end = content.find('[SECTION]', roadmap_start)
    if roadmap_end == -1:
        roadmap_end = content.find('## 5.', roadmap_start)
    if roadmap_end == -1:
        roadmap_end = roadmap_start + 5000
    
    roadmap_section = content[roadmap_start:roadmap_end]
    matches = re.findall(roadmap_pattern, roadmap_section, re.IGNORECASE)
    
    if not matches:
        return inserted
    
    # Detect phase for priority assignment
    phase_pattern = r'Phase\s*(\d+)|المرحلة\s*(\d+)'
    
    try:
        conn = get_db()
        current_phase = 1
        
        for activity, owner, timeline, deliverable in matches:
            title = activity.strip()[:200]
            owner_text = owner.strip()[:100]
            timeline_text = timeline.strip()[:100]
            deliverable_text = deliverable.strip()[:200]
            
            if not title or len(title) < 5:
                continue
            if title.lower() in ('activity', 'النشاط', '#', 'activity 1', 'activity 2'):
                continue
            if title.startswith('[') and title.endswith(']'):
                continue
            
            # Set priority based on timeline
            priority = 'high' if 'Month 1' in timeline_text or 'الشهر 1' in timeline_text else 'medium'
        ...
        if inserted:
            conn.commit()
            log_action(user_id, 'pipe_roadmap_to_tasks', {
                'domain': domain, 'count': len(inserted)
            })
            print(f"✅ MIZAN PIPE: Inserted {len(inserted)} implementation tasks from {domain} strategy roadmap", flush=True)
    except Exception as e:
        print(f"Mizan Pipe roadmap extraction error (non-fatal): {e}", flush=True)
...
def ensure_markdown_formatting(text):
    ...
    # ════════════════════════════════════════════════════════════════
    # PRE-FIX: Repair broken bold markers around Confidence Score
    # AI sometimes outputs: "Confidence Score:** 75% **Score Justification:"
    # where ** bold markers are shifted/misaligned.
    # Fix: Restore proper **Label:** formatting and split onto separate lines.
    # ════════════════════════════════════════════════════════════════
    
    # Fix broken pattern: "Confidence Score:** X% **Score Justification:" 
    # → "**Confidence Score:** X%\n\n**Score Justification:**"
    text = re.sub(
        r'(?:^|\n)\s*(?:\*\*)?Confidence Score:\*\*\s*(\d+%?)\s*\*\*Score Justification:\s*(?:\*\*)?',
        r'\n\n**Confidence Score:** \1\n\n**Score Justification:**',
        text
    )
    
    # Also fix Arabic variant
    text = re.sub(
        r'(?:^|\n)\s*(?:\*\*)?درجة الثقة:\*\*\s*(\d+%?)\s*\*\*(?:مبرر الدرجة|التبرير):\s*(?:\*\*)?',
        r'\n\n**درجة الثقة:** \1\n\n**مبرر الدرجة:**',
        text
    )
    
    # Fix: "Confidence Score:** X%" alone (missing opening **)
    text = re.sub(
    ...
    # Fix: "**Score Justification:" (missing closing **)
    text = re.sub(
        r'\*\*Score Justification:\s*(?!\*\*)',
    ...
    # B10b. Separate consecutive **Key:** value patterns onto own lines
    # Fixes: **Confidence Score:** 75% **Score Justification:** text
    # → **Confidence Score:** 75%
    #
    # **Score Justification:**
    # text
    ...
    # B10c. Ensure **Bold Label:** on its own line when preceded by other content
    # Handles: some text **Score Justification:** more text
    text = re.sub(
        r'([^\n*])\s+(\*\*(?:Score Justification|Confidence Score|درجة الثقة|مبرر الدرجة|التبرير):\*\*)',
        r'\1\n\n\2',
        text
    )
...
def api_generate_strategy():
    ...
    try:
        ...
        # ── Dynamic Confidence Score — available in both Arabic and English paths ──
        maturity_score_map = {
            'initial': 25, 'developing': 40, 'defined': 55, 'managed': 70, 'optimized': 85
        }
        base_confidence = maturity_score_map.get(maturity, 35)
        
        # ── Multi-Regulation Detection — available in both Arabic and English paths ──
        import re as _re_fw_pre
        fw_short_pre = _re_fw_pre.sub(r'\s*\([^)]+\)', '', frameworks_list).strip() or frameworks_list
        fw_list = [f.strip() for f in fw_short_pre.split(',') if f.strip()]
        is_multi_regulation = len(fw_list) > 1
...
{"تقييد الأُطر التنظيمية — أُطر متعددة مطلوبة:" if is_multi_regulation else "تقييد الإطار التنظيمي:"}
{"- المستخدم اختار " + str(len(fw_list)) + " أُطر تنظيمية: " + frameworks_ar if is_multi_regulation else "- الإطار المحدد هو: " + frameworks_ar}
{"- يجب ذكر كل إطار بمفرده في المبررات والأهداف — لا تجمعهم كتسمية واحدة" if is_multi_regulation else "- يجب الإشارة إلى " + frameworks_ar + " بالاسم في الرؤية والأهداف والركائز والمؤشرات"}
...
{"- اربط كل هدف بمتطلبات محددة من كل إطار على حدة: " + ' و '.join(fw_list) if is_multi_regulation else "- اربط كل هدف وكل ركيزة بمتطلبات محددة من " + frameworks_ar}
...
{"قاعدة حاسمة - عمود المبررات (أُطر متعددة):" if is_multi_regulation else "قاعدة حاسمة - عمود المبررات:"}
{"- يجب ذكر كل إطار بمفرده في كل مبرر — لا تدمج " + frameworks_ar + " كتسمية واحدة" if is_multi_regulation else "- المبرر يجب أن يشرح لماذا الهدف مهم وكيف يرتبط بمتطلب محدد من " + frameworks_ar}
{"- مثال صحيح: \"" + fw_list[0] + " يتطلب سياسات حوكمة رسمية؛ " + (fw_list[1] if len(fw_list) > 1 else fw_list[0]) + " يلزم بإجراءات التحكم في الوصول\"" if is_multi_regulation else "- مثال جيد: \"الخطأ البشري مسبب رئيسي للحوادث؛ " + frameworks_ar + " يلزم بتدريب أمني منتظم\""}
{"- مثال صحيح: \"الخطأ البشري مسبب رئيسي للحوادث؛ " + fw_list[0] + " يلزم بتدريب أمني؛ " + (fw_list[1] if len(fw_list) > 1 else fw_list[0]) + " يتطلب برامج توعية لمعالجي البيانات\"" if is_multi_regulation else "- مثال جيد: \"بدون فريق مختص لا يمكن تطبيق أي ضابط بشكل مستدام\""}
{"- مثال خاطئ: \"" + frameworks_ar + " يلزم بتدريب أمني\" ← هذا يعامل الأطر كتسمية واحدة!" if is_multi_regulation else "- مثال سيء: \"ضوابط الحماية التقنية\" (هذا وصف وليس مبرر)"}
...
**الرؤية الاستراتيجية:**
(فقرة أو فقرتان تصفان الحالة المستقبلية المرجوة للمنظمة فيما يتعلق بالامتثال لإطار {' و '.join(fw_list) if is_multi_regulation else frameworks_ar}. ركّز على النتائج والطموحات وليس المشاريع أو المبادرات)

### الأهداف الاستراتيجية:
{"| # | الهدف الاستراتيجي | المؤشر المستهدف | الإطار/الأُطر | المبرر (اذكر كل إطار بمفرده: " + ' و '.join(fw_list) + ") | الإطار الزمني |" if is_multi_regulation else "| # | الهدف الاستراتيجي | المؤشر المستهدف | المبرر (لماذا هذا مهم وما علاقته بـ " + frameworks_ar + ") | الإطار الزمني |"}
{"|---|-------------------|-----------------|--------------|----------------------------------------------------------|---------------|" if is_multi_regulation else "|---|-------------------|-----------------|------------------------------------------------------|---------------|"}
(5 أهداف - كل مبرر يشرح السبب الحقيقي وراء الهدف{"، ويذكر المتطلبات المحددة من كل إطار على حدة: " + ' و '.join(fw_list) if is_multi_regulation else " وعلاقته بمتطلب محدد من " + frameworks_ar}، وليس مجرد اسم ضابط)
...
### الفجوات المحددة:
{"| # | الفجوة | الوصف | الإطار/الأُطر | الأولوية | الحالة |" if is_multi_regulation else "| # | الفجوة | الوصف | الأولوية | الحالة |"}
{"|---|--------|-------|--------------|---------|--------|" if is_multi_regulation else "|---|--------|-------|---------|--------|"}
{gaps_table}
...
### مؤشرات الأداء:
{"| # | المؤشر | الوصف | الإطار/الأُطر | القيمة الحالية | القيمة المستهدفة | المبرر (اذكر متطلبات كل إطار على حدة) | الإطار الزمني | مصدر البيانات |" if is_multi_regulation else "| # | المؤشر | الوصف | القيمة الحالية | القيمة المستهدفة | المبرر (لماذا هذا المستهدف مهم) | الإطار الزمني | مصدر البيانات |"}
{"|---|--------|-------|--------------|----------------|------------------|------------------------------------------|---------------|---------------|" if is_multi_regulation else "|---|--------|-------|----------------|------------------|-------------------------------------|---------------|---------------|"}
(8 مؤشرات KPI - عمود "القيمة الحالية" يجب أن يكون "يُحدد بعد التقييم" لجميع المؤشرات بدون استثناء{" — يجب أن يُحدد كل مؤشر الإطار/الأُطر المرتبطة به: " + ' و '.join(fw_list) if is_multi_regulation else ""})
...
|---|--------|-------|---------|
(5 عوامل نجاح مرتبطة بمتطلبات {' و '.join(fw_list) + ' — اذكر كل إطار على حدة' if is_multi_regulation else frameworks_ar})
...
**تاريخ الإعداد:** يُحدد لاحقاً"""
            ...
            # ── Multi-Regulation Detection — reuse pre-computed fw_list and is_multi_regulation ──
            # fw_list and is_multi_regulation defined before lang branch
            
            if is_multi_regulation:
                # Build per-framework citation examples
                _fw_examples = ' and '.join(fw_list)
                _fw_cite_example = '; '.join([f'{fw} requires [specific control/clause]' for fw in fw_list])
                
                multi_fw_instruction = f"""MULTI-REGULATION REQUIREMENT — MANDATORY (DO NOT IGNORE):
The user has selected {len(fw_list)} regulatory frameworks: {fw_short}.
Every single row in the Objectives table, Gap table, and KPI table MUST explicitly name 
which framework(s) it maps to. Use the exact framework abbreviations: {', '.join(fw_list)}.

MANDATORY CITATION FORMAT in Justification/Description columns:
- If a row applies to one framework: "... per {fw_list[0]} [specific clause/domain]"
- If a row applies to multiple: "... per {fw_list[0]} [clause] and {fw_list[1]} [clause]"
- NEVER write just "{fw_short}" as a blob — always cite each framework INDIVIDUALLY with its specific requirement

EXAMPLE of CORRECT multi-framework objective row:
| 1 | Implement data classification controls | 100% of data assets classified | {fw_list[0]} mandates data classification in its data protection domain; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} requires data categorization for access control | Within 6 months |

EXAMPLE of WRONG (what you must NOT do):
| 1 | Implement controls | 100% coverage | {fw_short} compliance requirements | Within 6 months |
(This is wrong because it treats "{fw_short}" as one label and cites no specific clause from either framework)

The Gap Analysis MUST include gaps from EACH framework — not just the first one.
The KPI list MUST include KPIs that measure compliance with EACH framework separately."""
                
                # Build multi-reg table headers (adds Framework column)
                multi_obj_header = f"| # | Objective | Target Metric | Framework(s) | Justification (cite specific {_fw_examples} requirements) | Timeframe |"
                multi_obj_separator = "|---|-----------|---------------|--------------|----------------------------------------------------------|-----------|"
                multi_obj_example = f"| 1 | [Objective] | [Metric] | {fw_list[0]}, {fw_list[1] if len(fw_list) > 1 else fw_list[0]} | [Why this matters — cite specific requirement from EACH listed framework individually] | Within X months |"
                
                multi_gap_header = f"| # | Gap | Description | Applicable Framework(s) | Priority | Status |"
                multi_gap_separator = "|---|-----|-------------|------------------------|----------|--------|"
                
                multi_kpi_header = f"| # | KPI | Description | Framework(s) | Current Value | Target Value | Justification | Timeframe | Data Source |"
                multi_kpi_separator = "|---|-----|-------------|--------------|---------------|--------------|---------------|-----------|-------------|"
                multi_kpi_example = f"| 1 | [KPI Name] | [Brief description] | {fw_list[0]} | To be assessed | [Target] | [Cite specific {fw_list[0]} requirement] | Within X months | [Source] |"
            else:
                multi_fw_instruction = ""
                multi_obj_header = ""
                multi_obj_separator = ""
                multi_obj_example = ""
                multi_gap_header = ""
            ...
            # ── Justification rules — per-framework when multi-reg, single when not ──
            if is_multi_regulation:
                _fw0 = fw_list[0]
                _fw1 = fw_list[1] if len(fw_list) > 1 else fw_list[0]
                _justification_rules = f"""4. JUSTIFICATION COLUMN — MULTI-FRAMEWORK CITATIONS REQUIRED:
   Each justification must cite EACH applicable framework SEPARATELY by name.
   NEVER lump "{fw_short}" together as one label — always cite each framework INDIVIDUALLY.
   - CORRECT: "{_fw0} requires formal security policies (Governance domain); {_fw1} mandates documented access control procedures"
   - CORRECT: "Human error causes majority of incidents; {_fw0} mandates security training; {_fw1} requires awareness programs for data handlers"
   - WRONG: "{fw_short} mandates regular security training" ← treats both frameworks as one blob!
   - WRONG: "{fw_short} compliance requirements" ← no specific clause cited from either framework"""
            else:
                _justification_rules = f"""4. JUSTIFICATION COLUMN: Each justification must explain WHY the objective matters and HOW it connects to a specific {fw_short} requirement.
   ...
   - BAD: "{fw_short} awareness requirements" (this restates the objective, not why it matters)"""
            ...
            # ── Dynamic Confidence Score Instruction ──
            # (base_confidence already computed before lang branch)
            confidence_instruction = f"""CONFIDENCE SCORE — DYNAMIC CALCULATION:
Do NOT use a static score. Calculate the confidence score based on these ACTUAL inputs:
- Maturity Level: {maturity} (baseline: ~{base_confidence}%)
- Organization Size: {data.get('size', 'Medium')}
- Current Technologies: {tech_list}
- Organizational Structure: {org_structure}
- Budget: {data.get('budget', 'Not specified')}
Adjust the score UP for: higher maturity, existing technologies, dedicated teams, larger budgets.
Adjust the score DOWN for: initial maturity, no technologies, no dedicated team, limited budget.
The final score must reflect the REAL organizational readiness, not a default value."""
            
            # Build domain-specific English gap table (like Arabic prompt has)
            # When multi-regulation, gap descriptions cite each framework INDIVIDUALLY
            if is_multi_regulation:
                # Build per-framework gap descriptions instead of using fw_short as blob
                _fw_cite_all = ' and '.join(fw_list)  # "NCA CSCC and NCA DCC"
                domain_gaps_en = {
                    'Cyber Security': [
                        ('Policy & Governance Gap', f'Outdated or missing cybersecurity policies — {fw_list[0]} mandates formal governance framework; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} requires documented security policies', _fw_cite_all),
                        ('Technical Controls Gap', f'{fw_list[0]} requires implementation of technical security controls; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates additional data-centric technical safeguards', _fw_cite_all),
                        ('Training & Awareness Gap', f'Insufficient security awareness programs — required by both {fw_list[0]} (workforce training domain) and {fw_list[1] if len(fw_list) > 1 else fw_list[0]} (awareness controls)', _fw_cite_all),
                        ('Incident Response Gap', f'{fw_list[0]} requires incident response plan; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates breach notification procedures', _fw_cite_all),
                        ('Asset Management Gap', f'No comprehensive asset register — {fw_list[0]} mandates asset inventory; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} requires data asset classification', _fw_cite_all),
                    ],
                    'Data Management': [
                        ('Data Governance Gap', f'No formal data governance framework — required by {fw_list[0]}; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates governance roles', _fw_cite_all),
                        ('Data Quality Gap', f'No data quality standards — {fw_list[0]} requires quality measurement; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates data accuracy controls', _fw_cite_all),
                        ('Data Classification Gap', f'Data not classified — {fw_list[0]} requires sensitivity classification; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates criticality levels', _fw_cite_all),
                        ('Data Lifecycle Gap', f'Missing retention/disposal policies per {fw_list[0]}; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} requires lifecycle management', _fw_cite_all),
                        ('Data Protection Gap', f'Insufficient encryption and access controls per {fw_list[0]}; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates protection measures', _fw_cite_all),
                    ],
                }
                # Generic multi-reg gaps for domains not listed above
                _default_multi_gaps = [
                    ('Policy Gap', f'Missing or outdated policies — {fw_list[0]} requires documented policies; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates formal procedures', _fw_cite_all),
                    ('Awareness Gap', f'Insufficient training programs — required by {fw_list[0]} and {fw_list[1] if len(fw_list) > 1 else fw_list[0]}', _fw_cite_all),
                    ('Risk Management Gap', f'No risk methodology — {fw_list[0]} mandates risk assessment; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} requires risk treatment', _fw_cite_all),
                    ('Monitoring Gap', f'Limited oversight — {fw_list[0]} requires continuous monitoring; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} mandates audit controls', _fw_cite_all),
                    ('Continuity Gap', f'Incomplete continuity plans per {fw_list[0]}; {fw_list[1] if len(fw_list) > 1 else fw_list[0]} requires recovery procedures', _fw_cite_all),
                ]
                gaps = domain_gaps_en.get(domain, _default_multi_gaps)
            else:
                domain_gaps_en = {
                    'Cyber Security': [
            ...
            gaps_table_en = ""
            _is_ai_from_scratch = (
                domain in ('Artificial Intelligence', 'الذكاء الاصطناعي') and
                (maturity in ('initial', 'developing') or 
                 (org_structure and ('no' in org_structure.lower() or 'none' in org_structure.lower())))
            )
            
            if _is_ai_from_scratch:
                # FORCED: AI domain starting from scratch — governance + ethics are always #1 and #2
                if is_multi_regulation:
                    _fw_all = ', '.join(fw_list)
                    gaps_table_en += f"| 1 | Missing AI Governance Structure | No dedicated AI governance office, committee, or responsible executive — this is the prerequisite for all AI controls | {_fw_all} | Critical | Open - Confirmed |\n"
                    gaps_table_en += f"| 2 | Lack of Dedicated AI Ethics Committee | No independent AI ethics review board to oversee fairness, transparency, and accountability of AI systems | {_fw_all} | Critical | Open - Confirmed |\n"
                else:
                    gaps_table_en += "| 1 | Missing AI Governance Structure | No dedicated AI governance office, committee, or responsible executive — this is the prerequisite for all AI controls | Critical | Open - Confirmed |\n"
                    gaps_table_en += "| 2 | Lack of Dedicated AI Ethics Committee | No independent AI ethics review board to oversee fairness, transparency, and accountability of AI systems | Critical | Open - Confirmed |\n"
                start_idx = 3
                # Add remaining domain gaps starting from index 3 (skip first 2 which we replaced)
                remaining_gaps = gaps[2:] if len(gaps) > 2 else gaps
                for i, gap_tuple in enumerate(remaining_gaps, start_idx):
                    gap_name = gap_tuple[0]
                    gap_desc = gap_tuple[1]
                    gap_fw = gap_tuple[2] if len(gap_tuple) > 2 else fw_short
                    priority = "High" if i <= 4 else "Medium"
                    if is_multi_regulation:
                        gaps_table_en += f"| {i} | {gap_name} | {gap_desc} | {gap_fw} | {priority} | Open |\n"
                    else:
                        gaps_table_en += f"| {i} | {gap_name} | {gap_desc} | {priority} | Open |\n"
            elif org_structure and ('no' in org_structure.lower() or 'none' in org_structure.lower()):
                ...
                if is_multi_regulation:
                    gaps_table_en += f"| 1 | {gap_name} | {gap_desc} | {', '.join(fw_list)} | Critical | Open - Confirmed |\n"
                else:
            ...
            if not _is_ai_from_scratch:
                for i, gap_tuple in enumerate(gaps, start_idx):
                    gap_name = gap_tuple[0]
                    gap_desc = gap_tuple[1]
                    gap_fw = gap_tuple[2] if len(gap_tuple) > 2 else fw_short
...
{"FRAMEWORKS: " + fw_short if is_multi_regulation else "FRAMEWORK: " + fw_short}
{("- The user selected MULTIPLE frameworks: " + ', '.join(fw_list) + ". Reference each one INDIVIDUALLY by its abbreviation — NEVER combine them into one label.") if is_multi_regulation else '- Use "' + fw_short + '" naturally throughout — in objectives, gaps, KPIs, and pillars'}
...
DOCUMENT TITLE (must appear FIRST, before Section 1):
# {domain} Strategy — {data.get('org_name', 'Organization')}
**Prepared for:** {data.get('org_name', 'Organization')} | **Sector:** {data.get('sector', 'General')} | **Regulatory Framework:** {fw_short}
...
**Vision:**
[1-2 paragraphs: the organization's desired future state for {' and '.join(fw_list) + ' compliance' if is_multi_regulation else fw_short + ' compliance'}. Focus on OUTCOMES and aspirations — e.g., "become a trusted, resilient organization that…". This is a DESTINATION, not a roadmap. Do NOT list initiatives, projects, or workstreams here.]

### Strategic Objectives:
{multi_obj_header if is_multi_regulation else '| # | Objective | Target Metric | Justification (why it matters + link to ' + fw_short + ') | Timeframe |'}
{multi_obj_separator if is_multi_regulation else '|---|-----------|---------------|-----------------------------------------------------|-----------|'}
{multi_obj_example if is_multi_regulation else ('| 1 | Establish dedicated ' + _org_dept_en + ' with defined structure and authority | Approved org structure and full team | Without a dedicated team, no ' + fw_short + ' control can be sustainably implemented or monitored | Within 6 months |' if _has_no_org else '| 1 | [First objective] | [Metric] | [Why this matters + specific ' + fw_short + ' requirement] | Within X months |')}
(5-7 objectives total, each with a meaningful justification that explains the business need{' — EACH row must cite specific requirements from the individual frameworks: ' + ', '.join(fw_list) if is_multi_regulation else ''})
...
### Identified Gaps:
{multi_gap_header if is_multi_regulation else '| # | Gap | Description | Priority | Status |'}
{multi_gap_separator if is_multi_regulation else '|---|-----|-------------|----------|--------|'}
{gaps_table_en}
...
### KPIs:
{multi_kpi_header if is_multi_regulation else '| # | KPI | Description | Current Value | Target Value | Justification | Timeframe | Data Source |'}
{multi_kpi_separator if is_multi_regulation else '|---|-----|-------------|---------------|--------------|---------------|-----------|-------------|'}
{multi_kpi_example if is_multi_regulation else '| 1 | [KPI Name] | [Brief description] | To be assessed | [Justified value] | [Why this target] | Within X months | [Source] |'}
(8 KPIs total{' — EACH KPI must list the specific framework(s) it maps to: ' + ', '.join(fw_list) + '. Ensure at least 2 KPIs map to EACH framework.' if is_multi_regulation else ''})
...
**Confidence Score:** [Calculate based on organizational inputs above — maturity level, technologies, team structure, budget. Do NOT default to 75% or any static value. Show the actual calculated percentage.]%

**Score Justification:**
[Detailed paragraph explaining the basis for the confidence score, citing specific factors from the organization's inputs and their individual impact on the score]

### Critical Success Factors:
| # | Factor | Description | Importance |
|---|--------|-------------|------------|
(5 success factors linked to {' and '.join(fw_list) + ' requirements — cite each framework individually' if is_multi_regulation else fw_short + ' requirements'})
...
**Preparation Date:** To be determined"""
        ...
        # ── MULTI-REGULATION POST-PROCESSING: Ensure each framework is cited individually ──
        if is_multi_regulation and content:
            import re as _re_mfw
            
            # Count individual mentions of each framework (not as part of the combined label)
            fw_mention_counts = {}
            for fw in fw_list:
                # Count mentions that are NOT just part of the blob "NCA CSCC, NCA DCC"
                # A real individual citation is: "per NCA DCC" or "NCA DCC requires" or "NCA DCC domain"
                # Not: "NCA CSCC, NCA DCC" as a combined label
                individual_pattern = _re_mfw.escape(fw) + r'(?:\s+(?:requires|mandates|domain|controls?|clause|section|standard|framework|specifies|stipulates|addresses))'
                individual_count = len(_re_mfw.findall(individual_pattern, content, _re_mfw.IGNORECASE))
                fw_mention_counts[fw] = individual_count
            
            print(f"DEBUG: Multi-reg framework mention counts: {fw_mention_counts}", flush=True)
            
            # Find frameworks with ZERO individual citations (just appearing in the blob label)
            missing_fw = [fw for fw, count in fw_mention_counts.items() if count < 1]
            
            if missing_fw:
                print(f"DEBUG: ⚠️ Frameworks missing individual citations: {missing_fw}", flush=True)
                
                # REPAIR: Replace blob references with per-framework citations
                # Pattern: "per NCA CSCC, NCA DCC" → "per NCA CSCC and per NCA DCC"
                blob_label = ', '.join(fw_list)  # "NCA CSCC, NCA DCC"
                expanded_label = ' and '.join(fw_list)  # "NCA CSCC and NCA DCC"
                
                # Replace "per BLOB" with "per FW1 and per FW2"
                for prefix in ['per ', 'وفق ', 'حسب ', 'according to ', 'aligned with ', 'as required by ']:
                    old_blob = prefix + blob_label
                    new_expanded = prefix + expanded_label
                    if old_blob in content:
                        content = content.replace(old_blob, new_expanded)
                        print(f"DEBUG: Expanded blob reference: '{old_blob}' → '{new_expanded}'", flush=True)
                
                # Replace "BLOB requirements/compliance" with expanded
                for suffix in [' requirements', ' compliance', ' controls', ' mandates', ' standards']:
                    old_blob = blob_label + suffix
                    new_expanded = expanded_label + suffix
                    if old_blob in content:
                        content = content.replace(old_blob, new_expanded)
                
                # If a framework still has 0 mentions after expansion, inject note in gap section
                for fw in missing_fw:
                    if fw not in content or content.count(fw) < 3:
                        # Find the gap analysis section and append a framework-specific note
                        gap_marker = '## 3. Gap Analysis'
                        if gap_marker in content:
                            injection = f"\n\n> **Note:** {fw} compliance requirements must also be addressed across all identified gaps. {fw} mandates specific controls that complement the primary framework requirements.\n"
                            content = content.replace(gap_marker, gap_marker + injection, 1)
                            print(f"DEBUG: Injected {fw} compliance note into gap section", flush=True)
...
**درجة الثقة:** {base_confidence}%

**مبررات التقييم:**
تستند درجة الثقة إلى تقييم شامل للوضع الحالي للمنظمة في مجال {domain_val}، مع الأخذ بعين الاعتبار مستوى نضج العمليات الحالية ({maturity})، وتوفر الكوادر المؤهلة، ومدى جاهزية البنية التحتية التقنية. تعكس هذه الدرجة وجود فرص تحسين جوهرية مع الاعتراف بالتحديات التي تتطلب معالجة منهجية وفق متطلبات {fw_str if fw_str else 'الأطر التنظيمية المحددة'}.
...
**Confidence Score:** {base_confidence}%

**Score Justification:**
The confidence score is based on a comprehensive assessment of the organization's current state in {domain_val}, with maturity level at "{maturity}". This considers the maturity level of existing processes, availability of qualified personnel, and technical infrastructure readiness. This score reflects substantial improvement opportunities while acknowledging challenges that require systematic remediation per {fw_str if fw_str else 'selected regulatory frameworks'}.
...
"""
        ...
        # ── MIZAN PIPE: Auto-extract risks, initiatives, KPIs, gaps, and roadmap from AI output ──
        pipe_domain = data.get('domain', 'General')
        try:
            risk_ids = pipe_risks_to_register(content, session['user_id'], pipe_domain)
            initiative_ids = pipe_initiatives_to_projects(content, session['user_id'], pipe_domain)
            kpi_ids = pipe_kpis_to_tasks(content, session['user_id'], pipe_domain)
            gap_ids = pipe_gaps_to_tasks(content, session['user_id'], pipe_domain)
            roadmap_ids = pipe_roadmap_to_tasks(content, session['user_id'], pipe_domain)
        except Exception as pipe_err:
            print(f"Mizan Pipe error (non-fatal): {pipe_err}", flush=True)
            risk_ids, initiative_ids, kpi_ids, gap_ids, roadmap_ids = [], [], [], [], []
        ...
        log_action(session['user_id'], 'generate_strategy', {
            ...
            'pipe_risks': len(risk_ids) if risk_ids else 0,
            'pipe_initiatives': len(initiative_ids) if initiative_ids else 0,
            'pipe_kpis': len(kpi_ids) if kpi_ids else 0,
            'pipe_gaps': len(gap_ids) if gap_ids else 0,
            'pipe_roadmap': len(roadmap_ids) if roadmap_ids else 0,
        })
        ...
        return jsonify({
            ...
            'sections': sections,
            'pipe': {
                'risks_added': len(risk_ids) if risk_ids else 0,
                'initiatives_added': len(initiative_ids) if initiative_ids else 0,
                'kpis_added': len(kpi_ids) if kpi_ids else 0,
                'gaps_added': len(gap_ids) if gap_ids else 0,
                'roadmap_added': len(roadmap_ids) if roadmap_ids else 0,
            },
...
def api_risk_register_list():
    ...
    Unified view: returns risks from ALL domains by default.
    Optional ?domain=Cyber+Security to filter by a specific domain.
    Optional ?source=ai_strategy to filter by source type.
    """
    try:
        conn = get_db()
        ...
        query = 'SELECT * FROM risk_register WHERE user_id = ?'
        ...
        query += ' ORDER BY (likelihood * impact) DESC, created_at DESC'
        
        risks = conn.execute(query, params).fetchall()
        
        # Also compute domain summary for the unified view
        domain_summary = conn.execute('''
            SELECT COALESCE(domain, 'General') as domain, COUNT(*) as count,
                   ROUND(AVG(likelihood * impact), 1) as avg_score
            FROM risk_register WHERE user_id = ?
...
def api_risk_register_create():
    ...
    try:
        ...
        conn = get_db()
        conn.execute('''
            INSERT INTO risk_register (user_id, name, description, category, likelihood, impact, owner, treatment, treatment_plan, status, domain, source_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'manual')
        ''', (
            session['user_id'],
            name,
...
"""
        ...
        # ── MIZAN PIPE: Auto-extract risk into Risk Register + controls as tasks ──
        pipe_risk_ids = []
        try:
            pipe_risk_ids = pipe_risk_assessment_to_register(
                content, session['user_id'], data.get('domain', 'General'),
                asset=data.get('asset', ''), threat=data.get('threat', '')
            )
        except Exception as pipe_err:
            print(f"Mizan Pipe risk-assessment error (non-fatal): {pipe_err}", flush=True)
...
def api_generate_docx():
    ...
    try:
        ...
        try:
            ...
            # Set Arabic font — Noto Sans Arabic for consistency
            if is_arabic:
                rPr = style.element.get_or_add_rPr()
                rFonts = parse_xml(f'<w:rFonts {nsdecls("w")} w:cs="Noto Sans Arabic" w:ascii="Calibri" w:hAnsi="Calibri"/>')
                rPr.append(rFonts)
        except Exception:
            pass
        ...
        try:
            for level in [1, 2, 3]:
                ...
                # Set Arabic font on headings too
                if is_arabic:
                    h_rPr = h_style.element.get_or_add_rPr()
                    h_rFonts = parse_xml(f'<w:rFonts {nsdecls("w")} w:cs="Noto Sans Arabic" w:ascii="Calibri" w:hAnsi="Calibri"/>')
    ...
    try:
        ...
        if is_arabic:
            try:
                ...
                # Extended font search paths — Noto Sans Arabic is the STANDARD Arabic font
                font_paths = [
                    # Noto Sans Arabic — primary (matches UI and DOCX)
                    '/usr/share/fonts/truetype/noto/NotoSansArabic-Regular.ttf',
                    '/usr/share/fonts/opentype/noto/NotoSansArabic-Regular.ttf',
                    'static/fonts/NotoSansArabic-Regular.ttf',
                    # Noto Naskh Arabic — secondary
                    '/usr/share/fonts/truetype/noto/NotoNaskhArabic-Regular.ttf',
                    # FreeFonts (has Arabic glyphs)
                ...
                # Also search for Noto Sans Arabic font using glob
                additional_paths = glob.glob('/usr/share/fonts/**/[Nn]oto*[Ss]ans*[Aa]rabic*.ttf', recursive=True)
        ...
        def normalize_markdown_for_pdf(raw):
            ...
            raw = _re_norm.sub(r'(?:^|\n)\s*Confidence Score:\*\*\s*(\d+%)', r'\n**Confidence Score:** \1', raw)
            raw = _re_norm.sub(r'\*\*Score Justification:\s*(?!\*\*)', r'**Score Justification:** ', raw)
            raw = _re_norm.sub(r'\*\*مبرر الدرجة:\s*(?!\*\*)', r'**مبرر الدرجة:** ', raw)
...
def api_get_tasks():
    ...
    Excludes risk_mitigation tasks by default (those belong in Risk Register).
    Pass ?include_risk_mitigation=true to include them.
    """
    try:
        domain = request.args.get('domain', '')
        status = request.args.get('status', '')
        include_risk = request.args.get('include_risk_mitigation', 'false').lower() == 'true'
        
        conn = get_db()
        query = 'SELECT * FROM project_tasks WHERE user_id = ?'
        params = [session['user_id']]
        
        # Exclude risk mitigation tasks from project management by default
        if not include_risk:
            query += ' AND (category IS NULL OR category != ?)'
            params.append('risk_mitigation')
...
@login_required
def api_task_stats():
    """Get task statistics (excludes risk_mitigation tasks — those are in Risk Register)."""
    try:
        conn = get_db()
        user_id = session['user_id']
        
        _excl = ' AND (category IS NULL OR category != "risk_mitigation")'
        todo = conn.execute('SELECT COUNT(*) FROM project_tasks WHERE user_id=? AND status="todo"' + _excl, (user_id,)).fetchone()[0]
        in_progress = conn.execute('SELECT COUNT(*) FROM project_tasks WHERE user_id=? AND status="in_progress"' + _excl, (user_id,)).fetchone()[0]
        done = conn.execute('SELECT COUNT(*) FROM project_tasks WHERE user_id=? AND status="done"' + _excl, (user_id,)).fetchone()[0]
        blocked = conn.execute('SELECT COUNT(*) FROM project_tasks WHERE user_id=? AND status="blocked"' + _excl, (user_id,)).fetchone()[0]
        
        overdue = conn.execute(
            'SELECT COUNT(*) FROM project_tasks WHERE user_id=? AND status!="done" AND due_date < date("now") AND due_date != ""' + _excl,
            (user_id,)
        ).fetchone()[0]
...
def api_tasks_by_category():
    ...
    Categories: strategic_initiative, implementation, kpi, gap_mitigation
    ...
    try:
        ...
        count_query = '''SELECT COALESCE(category, 'implementation') as cat, COUNT(*) as cnt 
                        FROM project_tasks WHERE user_id = ?'''
        count_params = [user_id]
        if domain:
            count_query += ' AND domain = ?'
            count_params.append(domain)
        count_query += ' GROUP BY COALESCE(category, "implementation")'
        
        counts = conn.execute(count_query, count_params).fetchall()
        category_counts = {r['cat']: r['cnt'] for r in counts}
