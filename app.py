...
def api_generate_strategy():
    ...
    try:
        ...
        if lang == 'ar':
            ...
            # Build gaps table
            gaps_table = ""
            # ── AI "from scratch" detection — Arabic ──
            _is_ai_from_scratch_ar = (
                domain in ('Artificial Intelligence', 'الذكاء الاصطناعي') and
                (maturity in ('initial', 'developing') or 
                 (org_structure and ('no' in org_structure.lower() or 'لا' in org_structure or 'none' in org_structure.lower())))
            )
            
            if _is_ai_from_scratch_ar:
                # FORCED: AI domain starting from scratch — governance + ethics are always #1 and #2
                gaps_table += "| 1 | غياب هيكل حوكمة الذكاء الاصطناعي | عدم وجود مكتب حوكمة AI أو لجنة أو مسؤول تنفيذي مختص — هذا شرط أساسي لجميع ضوابط AI | حرجة | مفتوحة - مؤكدة |\n"
                gaps_table += "| 2 | غياب لجنة أخلاقيات AI مستقلة | عدم وجود مجلس مراجعة أخلاقيات مستقل للإشراف على العدالة والشفافية والمساءلة لأنظمة AI | حرجة | مفتوحة - مؤكدة |\n"
                start_idx = 3
                for i, (gap_name, gap_desc) in enumerate(gaps[2:], start_idx):
                    priority = "عالية" if i <= 4 else "متوسطة"
            ...
            elif org_structure and ('no' in org_structure.lower() or 'لا' in org_structure or 'none' in org_structure.lower()):
...
# {data.get('org_name', 'المنظمة')} — استراتيجية {domain_desc}
...
{"متطلب تعدد الأنظمة — حاسم:" + chr(10) + "المستخدم اختار أكثر من إطار تنظيمي: " + frameworks_ar + "." + chr(10) + "يجب إنتاج تحليل مُوحّد يغطي جميع الأطر المختارة:" + chr(10) + "- تحليل الفجوات: حدد الفجوات مقابل كل إطار. إذا كانت فجوة تنطبق على أكثر من إطار، اذكرها مرة واحدة مع جميع الأطر المعنية." + chr(10) + "- المؤشرات: أنشئ مؤشرات موحدة تقيس الامتثال عبر جميع الأطر." + chr(10) + "- الأهداف: كل هدف يجب أن يرتبط بمتطلبات محددة من الأطر المعنية." + chr(10) + "- لا تركز على الإطار الأول فقط — أعطِ وزناً متساوياً لجميع الأطر." if len([f.strip() for f in frameworks_ar.split('،') if f.strip()]) > 1 or len([f.strip() for f in frameworks_ar.split(',') if f.strip()]) > 1 else ""}

حساب درجة الثقة — ديناميكي:
لا تستخدم درجة ثابتة. احسب درجة الثقة بناءً على المدخلات الفعلية:
- مستوى النضج: {maturity} (الأساس: ~{base_confidence}%)
- حجم المنظمة: {data.get('size', 'متوسطة')}
- التقنيات الحالية: {', '.join(technologies) if technologies else 'غير محددة'}
- الهيكل التنظيمي: {org_structure}
- الميزانية: {data.get('budget', 'غير محددة')}
ارفع الدرجة: نضج أعلى، تقنيات موجودة، فريق مختص، ميزانية كافية.
اخفض الدرجة: نضج أولي، لا تقنيات، لا فريق مختص، ميزانية محدودة.
...
**تاريخ الإعداد:** يُحدد لاحقاً"""
            ...
            # ── Multi-Regulation Detection ──
            # If user selected multiple frameworks, build combined analysis instructions
            fw_list = [f.strip() for f in fw_short.split(',') if f.strip()]
            is_multi_regulation = len(fw_list) > 1
            
            if is_multi_regulation:
                multi_fw_instruction = f"""MULTI-REGULATION REQUIREMENT — CRITICAL:
The user has selected MULTIPLE regulatory frameworks: {fw_short}.
You MUST produce a COMBINED analysis that covers ALL selected frameworks:
- Gap Analysis: Identify gaps against EACH framework. If a gap applies to multiple frameworks, list it once with all applicable frameworks noted.
- KPI List: Create UNIFIED KPIs that measure compliance across ALL frameworks. Each KPI must reference which framework(s) it satisfies.
- Objectives: Each objective must map to specific requirements from the relevant framework(s).
- Do NOT focus only on the first framework — give equal weight to all."""
            else:
...
# {data.get('org_name', 'Organization')} — {domain} Strategy Document
...
{"6. AI FROM SCRATCH — PRIORITY OVERRIDE: The organization is starting AI governance from scratch (maturity: " + maturity + "). The #1 gap MUST be 'Missing AI Governance Structure' and #2 MUST be 'Lack of Dedicated AI Ethics Committee'. The first strategic pillar must focus entirely on establishing these foundational elements before any technical AI initiatives." if _is_ai_from_scratch else ""}
