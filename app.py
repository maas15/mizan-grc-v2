...
(4-5 risks)"""
        ...
        def identify_section(text, lang_code):
            ...
            if first_line.startswith('## 1.') or first_line.startswith('1.') or 'vision' in first_line:
                return 'vision'
            if first_line.startswith('## 2.') or (first_line.startswith('2.') and 'gap' in first_line):
                return 'gaps'
            if first_line.startswith('## 3.') or (first_line.startswith('3.') and 'pillar' in first_line):
                return 'pillars'
            if first_line.startswith('## 4.') or (first_line.startswith('4.') and ('roadmap' in first_line or 'implementation' in first_line)):
                return 'roadmap'
            if first_line.startswith('## 5.') or (first_line.startswith('5.') and 'kpi' in first_line):
                return 'kpis'
            if first_line.startswith('## 6.') or (first_line.startswith('6.') and 'confidence' in first_line):
                return 'confidence'
            
            # Arabic section numbers
            if '1. الرؤية' in first_line or 'الرؤية والأهداف' in first_line:
                return 'vision'
            if '2. تحليل' in first_line or 'تحليل الفجوات' in first_line:
                return 'gaps'
            if '3. الركائز' in first_line:
                return 'pillars'
            if '4. خارطة' in first_line or 'خارطة الطريق' in first_line:
                return 'roadmap'
            if '5. مؤشرات' in first_line:
                return 'kpis'
            if '6. تقييم' in first_line:
                return 'confidence'
            
            # Fallback to keyword matching in full text
            keyword_scores = {
                'vision': ['vision', 'objective', 'mission', 'الرؤية', 'الأهداف'],
                'gaps': ['gap analysis', 'identified gaps', 'الفجوة', 'الفجوات'],
                'pillars': ['pillar', 'initiative', 'الركائز', 'المبادرات'],
                'roadmap': ['phase 1', 'roadmap', 'timeline', 'implementation', 'المرحلة', 'خارطة'],
                'kpis': ['kpi', 'key performance', 'indicator', 'مؤشر', 'مؤشرات'],
                'confidence': ['confidence score', 'confidence assessment', 'الثقة', 'تقييم الثقة']
            }
