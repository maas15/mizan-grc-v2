...
def generate_simulation_content(prompt, language='en'):
    ...
    if not fw_match:
        # Try English - look for "Selected Frameworks (USE ONLY THESE):" or "selected these specific frameworks:"
        fw_match = re.search(r'(?:Selected Frameworks[^:]*|selected these specific frameworks?):\s*([^\n]+)', prompt, re.IGNORECASE)
    if fw_match:
        framework = fw_match.group(1).strip().rstrip(',')
    ...
    if '[section]' in prompt_lower or '6 separate sections' in prompt_lower or 'vision & objective' in prompt_lower or 'الرؤية والأهداف' in prompt_lower or 'استراتيجية شاملة' in prompt_lower:
        return generate_strategy_simulation(language, framework)
...
def generate_strategy_simulation(language='en', framework='NCA ECC'):
    """Generate strategy simulation content with implementation guidelines."""
    fw = framework  # Short alias
    if language == 'ar':
...
|---|-------|----------------|------------------------|---------------|
| 1 | تحقيق الامتثال الكامل لإطار {fw} | امتثال > 95% | متطلب أساسي من ضوابط {fw} | خلال 12 شهر |
| 2 | تعزيز قدرات الكشف والاستجابة | تقليل وقت الاستجابة 50% | ضوابط الكشف والاستجابة في {fw} | خلال 18 شهر |
| 3 | تطوير برنامج توعية شامل | تغطية 100% من الموظفين | متطلبات التوعية والتدريب في {fw} | خلال 6 أشهر |
| 4 | تطبيق الضوابط التقنية المطلوبة وفق {fw} | تطبيق جميع الضوابط الأساسية | ضوابط الحماية التقنية في {fw} | خلال 12 شهر |
| 5 | إنشاء فريق استجابة مركزي | فريق عمل 24/7 | متطلبات إدارة الحوادث في {fw} | خلال 9 أشهر |
...
|---|--------|-------|----------|--------|
| 1 | فجوة السياسات والحوكمة | الحاجة لتحديث السياسات لتتوافق مع متطلبات {fw} | عالية | يحتاج تقييم |
| 2 | فجوة الضوابط التقنية | نقص في تطبيق الضوابط التقنية المطلوبة وفق {fw} | عالية | يحتاج تقييم |
| 3 | فجوة التدريب والتوعية | برامج توعية غير كافية للموظفين | متوسطة | يحتاج تقييم |
| 4 | فجوة الاستجابة للحوادث | خطة استجابة للحوادث غير مكتملة وفق متطلبات {fw} | عالية | يحتاج تقييم |
| 5 | فجوة إدارة الأصول | عدم وجود سجل شامل ومحدث للأصول السيبرانية | متوسطة | يحتاج تقييم |


---

### دليل تنفيذ الفجوة رقم 1: فجوة السياسات والحوكمة
...
| التخطيط | 1.2 | تحديد متطلبات {fw} المتعلقة بالحوكمة | الامتثال | جدول المتطلبات |
...
### الركيزة 1: الامتثال والحوكمة (وفق {fw})
• تطوير إطار شامل لإدارة الامتثال لمتطلبات {fw}
• إنشاء فريق مراقبة مستمرة للامتثال
• تحديث السياسات والإجراءات بشكل دوري لمواكبة تحديثات {fw}
...
|---|--------|---------|--------|
| 1 | مراجعة السياسات وتحليل الفجوات وفق {fw} | أمن المعلومات | الشهر 2 |
| 2 | بدء برامج التدريب الأساسية | الموارد البشرية | الشهر 3 |
| 3 | تطبيق الضوابط التقنية ذات الأولوية العالية | تقنية المعلومات | الشهر 6 |
...
|---|--------|---------------|-----------------|---------------|
| 1 | نسبة الامتثال لإطار {fw} | يُحدد بعد التقييم | > 95% | خلال 12 شهر |
| 2 | وقت الاستجابة للحوادث | يُحدد بعد التقييم | < 1 ساعة | خلال 12 شهر |
| 3 | معدل إكمال التدريب | يُحدد بعد التقييم | > 90% | خلال 6 أشهر |
| 4 | تقليل الحوادث الأمنية الناجحة | يُحدد بعد التقييم | تقليل 40% | خلال 18 شهر |
| 5 | معدل نجاح التدقيق | يُحدد بعد التقييم | > 95% | خلال 12 شهر |
| 6 | نسبة تطبيق الضوابط التقنية وفق {fw} | يُحدد بعد التقييم | 100% | خلال 12 شهر |
| 7 | تغطية المصادقة متعددة العوامل (MFA) | يُحدد بعد التقييم | 100% | خلال 6 أشهر |
| 8 | معدل دقة التنبيهات الأمنية | يُحدد بعد التقييم | > 85% | خلال 18 شهر |
...
|---|-----------|---------------|----------------------|-----------|
| 1 | Achieve full compliance with {fw} | Compliance > 95% | Core requirement of {fw} controls | Within 12 months |
| 2 | Enhance detection and response capabilities | Reduce response time 50% | {fw} detection and response controls | Within 18 months |
| 3 | Develop comprehensive awareness program | 100% employee coverage | {fw} awareness and training requirements | Within 6 months |
| 4 | Implement mandatory technical controls per {fw} | All essential controls applied | {fw} technical protection controls | Within 12 months |
| 5 | Establish centralized incident response team | 24/7 operations | {fw} incident management requirements | Within 9 months |
...
|---|-----|-------------|----------|--------|
| 1 | Policy & Governance Gap | Need to update policies to meet {fw} requirements | High | To Be Assessed |
| 2 | Technical Controls Gap | Gaps in implementing mandatory technical controls per {fw} | High | To Be Assessed |
| 3 | Training & Awareness Gap | Insufficient awareness programs for employees | Medium | To Be Assessed |
| 4 | Incident Response Gap | Incomplete incident response plan per {fw} requirements | High | To Be Assessed |
| 5 | Asset Management Gap | No comprehensive and updated cybersecurity asset register | Medium | To Be Assessed |


---

### Gap #1 Implementation Guide: Policy & Governance Gap
...
| Planning | 1.2 | Identify {fw} governance requirements | Compliance | Requirements matrix |
...
### Pillar 1: Compliance & Governance (per {fw})
• Develop comprehensive compliance management framework for {fw}
• Establish continuous compliance monitoring team
• Regular policy and procedure updates to align with {fw} revisions
...
|---|----------|-------|----------|
| 1 | Policy review and gap analysis against {fw} | InfoSec | Month 2 |
| 2 | Begin basic training programs | HR | Month 3 |
| 3 | Implement high-priority technical controls | IT | Month 6 |
...
|---|-----|---------------|--------------|-----------|
| 1 | {fw} compliance rate | To be assessed | > 95% | Within 12 months |
...
| 6 | {fw} technical controls implementation | To be assessed | 100% | Within 12 months |
| 7 | MFA coverage | To be assessed | 100% | Within 6 months |
| 8 | Security alert accuracy rate | To be assessed | > 85% | Within 18 months |
...
**النتيجة العامة:** يُحدد بعد استكمال التقييم الميداني
...
**Overall Result:** To be determined after field assessment
...
| التشغيل | 4.3 | ضبط التنبيهات لتقليل التنبيهات الخاطئة | فريق الأمن | تنبيهات دقيقة |
...
{"- الهيكل التنظيمي الحالي لا يتضمن إدارة مختصة بالأمن السيبراني - يجب أن تتضمن الاستراتيجية هدفاً وركيزة لإنشاء إدارة أو قسم مختص بالأمن السيبراني مع تحديد الهيكل المقترح والصلاحيات" if org_structure and ('no' in org_structure.lower() or 'لا' in org_structure or 'not' in org_structure.lower() or 'none' in org_structure.lower()) else ""}
...
### الأهداف الاستراتيجية:
| # | الهدف الاستراتيجي | المؤشر المستهدف | المبرر (مرتبط بـ {frameworks_ar}) | الإطار الزمني |
|---|-------------------|-----------------|-------------------------------------|---------------|
(5 أهداف مع ربط كل هدف بمتطلب محدد من {frameworks_ar})
...
### مؤشرات الأداء:
| # | المؤشر | الوصف | القيمة الحالية | القيمة المستهدفة | المبرر (مرتبط بـ {frameworks_ar}) | الإطار الزمني | مصدر البيانات |
|---|--------|-------|----------------|------------------|-------------------------------------|---------------|---------------|
...
{"- IMPORTANT: The organization currently has NO dedicated cybersecurity department/structure. The strategy MUST include an objective and pillar for ESTABLISHING a cybersecurity department with proposed structure, roles, and authority." if org_structure and ('no' in org_structure.lower() or 'none' in org_structure.lower()) else ""}
...
تقييد مهم:
- هذه السياسة يجب أن تكون خاصة بموضوع "{policy_name}" فقط وحصراً
- جميع البنود والإجراءات يجب أن تتعلق مباشرة بـ "{policy_name}"
- لا تكتب بنوداً عامة لا علاقة لها بموضوع السياسة
- الإطار التنظيمي المرجعي: {framework_ar}
- لا تذكر أي إطار أو معيار آخر غير {framework_ar} إلا إذا كان مذكوراً صراحةً أعلاه
...
4. اكتب بنود السياسة بشكل مفصل ومحدد لموضوع {policy_name}

# سياسة {policy_name}

## 1. الغرض
(وصف الغرض من هذه السياسة بشكل خاص بموضوع {policy_name})

## 2. النطاق
تنطبق هذه السياسة على:
- (قائمة بالأطراف المعنية بموضوع {policy_name})

## 3. بنود السياسة
### 3.1 (عنوان فرعي متعلق بـ {policy_name})
- (بنود مفصلة ومحددة)
### 3.2 (عنوان فرعي آخر)
- (بنود مفصلة)
### 3.3 (عنوان فرعي إضافي)
- (بنود مفصلة)
...
- (متطلبات الامتثال المرتبطة بإطار {framework_ar} فقط)
...
- This policy MUST be entirely and specifically about "{policy_name}" - every section and statement must directly relate to this topic
- Do NOT write generic security policy content unrelated to {policy_name}
- Reference framework: {framework}
- Do NOT mention any other framework or standard (like ISO 27001, NIST, COBIT) unless it is the selected framework above

INSTRUCTIONS:
1. Do NOT use any specific dates (like 2024, 2025, January, etc.)
2. Do NOT use any person names
3. Use ONLY relative timeframes like: "Annually", "Every 90 days", "Within 1 year"
4. For dates use: [To be added upon approval]
5. Write detailed policy statements specific to {policy_name}

# {policy_name} Policy

## 1. Purpose
(Description specific to {policy_name})

## 2. Scope
This policy applies to:
- (List of stakeholders relevant to {policy_name})

## 3. Policy Statements
### 3.1 (Subheading related to {policy_name})
- (Detailed statements)
### 3.2 (Another subheading)
- (Detailed statements)
### 3.3 (Additional subheading)
- (Detailed statements)
...
- (Requirements linked to {framework} only)
...
## Executive Summary
Brief overview of audit results - overall compliance status against {framework} requirements
