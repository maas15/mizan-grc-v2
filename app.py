...
def repair_table_rows(text):
    ...
    while i < len(lines):
        ...
        if table_col_count > 0:
            # Standard table row starting with |
            if s.startswith('|') and s.endswith('|') and '---' not in s:
                cells = [c.strip() for c in s.split('|')[1:-1]]
                # Check if empty data row (all cells except # are empty)
                content_cells = cells[1:] if has_num_col else cells
                non_empty = sum(1 for c in content_cells if c and len(c.strip()) > 0)
                if non_empty == 0:
                    # Skip empty row
                    i += 1
                    continue
...
- لا تُخرج أبداً صفوف جدول فارغة — كل خلية في كل صف يجب أن تحتوي على محتوى
- خطأ (خلايا فارغة): | 1 | | | |
- خطأ (فواصل متكررة): ---|---|---|---| | 1 |
- خطأ (فاصل لكل صف): |---|---| ثم |---|---| ثم |---|---|
- صحيح: | 1 | محتوى هنا | المزيد | المخرج |
- صف الفاصل يستخدم |---| ويظهر مرة واحدة فقط بعد صف الرأس
- صفوف البيانات تحتوي على محتوى فقط — لا تضع شرطات كقيم خلايا
- كل صف يجب أن يبدأ بـ | وينتهي بـ | — بلا استثناء
...
|---|---------|-------|---------------|
| 1 | إطار الحوكمة | تطوير هيكل حوكمة شامل | ميثاق الحوكمة |
| 2 | منظومة السياسات | إعداد 25+ سياسة معتمدة | مكتبة السياسات |
| 3 | إدارة المخاطر | تنفيذ تقييم المخاطر | سجل المخاطر |
...
- The separator row (line 2 with dashes) appears ONLY ONCE per table, immediately after the header
- Each table row MUST be on its own line — NEVER merge multiple rows onto one line
- NEVER output empty table rows — every cell in every row MUST have content
- WRONG (empty cells): | 1 | | | |
- WRONG (repeated separators): ---|---|---|---| | 1 |
- WRONG (separator per row): |---|---| then |---|---| then |---|---|
- RIGHT: | 1 | Content here | More content | Deliverable |
- The separator row uses |---| format and appears EXACTLY ONCE after the header row
- Data rows contain ONLY content — never dashes as cell values
- Every row MUST start with | and end with | — no exceptions
- Tables MUST have the same number of | delimiters in EVERY row (header, separator, and data)
...
|---|-----------|-------------|-------------|
| 1 | Risk Framework | Implement risk assessment | Risk register |
| 2 | Policy Suite | Develop 25+ policies | Policy library |
| 3 | Board Governance | Establish steering committee | Committee charter |
...
def api_generate_docx():
    ...
    try:
        ...
        # Strip AI-generated metadata table (download adds its own via title/filename)
        content = re.sub(r'^\s*\|\s*\|\s*\|\s*\n\|---\|---\|\s*\n(?:\|[^\n]*\|\s*\n)*', '', content, flags=re.MULTILINE)
        content = re.sub(r'^\s*\|\s*\*\*(?:Document Title|عنوان الوثيقة)\*\*\s*\|[^\n]*\n(?:\|[^\n]*\n)*', '', content, flags=re.MULTILINE)
