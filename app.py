...
def build_framework_procedures(domain_code, lang='en'):
    """Generate procedure templates for EVERY framework in a domain."""
    ...
    for region_or_cat, fw_list in frameworks_dict.items():
        for fw_name in fw_list:
            ...
            if lang == 'en':
                procedures[f"{short} - Compliance Assessment Procedure"] = {
                    "description": f"Procedure for assessing current compliance status against {full_desc} mandatory requirements, identifying gaps, and creating remediation roadmap",
                    "framework": fw_name
                }
                procedures[f"{short} - Controls Implementation Procedure"] = {
                    "description": f"Procedure for implementing mandatory controls, technical requirements, and operational procedures specified by {full_desc}",
                    "framework": fw_name
                }
                procedures[f"{short} - Audit & Monitoring Procedure"] = {
                    "description": f"Procedure for conducting periodic compliance audits, continuous monitoring, evidence collection, and reporting for {full_desc}",
                    "framework": fw_name
                }
            else:  # Arabic
                procedures[f"إجراء تقييم الامتثال - {short}"] = {
                    "description": f"إجراء لتقييم حالة الامتثال الحالية مقابل متطلبات {full_desc} الإلزامية وتحديد الفجوات وإنشاء خارطة طريق المعالجة",
                    "framework": fw_name
                }
                procedures[f"إجراء تطبيق الضوابط - {short}"] = {
                    "description": f"إجراء لتطبيق الضوابط الإلزامية والمتطلبات التقنية والإجراءات التشغيلية المحددة في {full_desc}",
                    "framework": fw_name
                }
                procedures[f"إجراء التدقيق والمراقبة - {short}"] = {
                    "description": f"إجراء لإجراء تدقيقات الامتثال الدورية والمراقبة المستمرة وجمع الأدلة والإبلاغ لـ {full_desc}",
                    "framework": fw_name
                }
...
# ── Merge framework procedures into DOMAIN_TEMPLATES ──
def merge_framework_procedures():
    """Add dynamically generated framework procedures to DOMAIN_TEMPLATES."""
    for dc in ['cyber', 'data', 'ai', 'dt', 'global', 'erm']:
        if dc not in DOMAIN_TEMPLATES:
            DOMAIN_TEMPLATES[dc] = {}
        if 'procedure' not in DOMAIN_TEMPLATES[dc]:
            DOMAIN_TEMPLATES[dc]['procedure'] = {'en': {}, 'ar': {}}
        
        for lang in ['en', 'ar']:
            generated = build_framework_procedures(dc, lang)
            if lang not in DOMAIN_TEMPLATES[dc]['procedure']:
                DOMAIN_TEMPLATES[dc]['procedure'][lang] = {}
            DOMAIN_TEMPLATES[dc]['procedure'][lang].update(generated)
...
# ── Execute framework merges (must run after DOMAIN_TEMPLATES and AWARENESS_MODULES are defined) ──
merge_framework_procedures()
...
def ensure_markdown_formatting(text):
    ...
    # e.g., "## 3. Strategic Pillars ### Pillar 1:" → separate lines
    text = re.sub(r'(##\s+\d*\.?\s*[^\n#]+?)\s+(###\s+)', r'\1\n\n\2', text)
    
    # 2. Split ## heading from **bold:** merged on same line
    # e.g., "## 1. Vision & Objectives **Vision:**" → separate lines
    text = re.sub(r'(##\s+\d*\.?\s*[^\n*]+?)\s+(\*\*[^*]+\*\*)', r'\1\n\n\2', text)
    ...
    # 11. Fix consecutive blank lines (max 2)
    text = re.sub(r'\n{4,}', '\n\n\n', text)
...
**Owner:** [Responsible Department]"""
        ...
        if is_procedure:
            prompt = prompt.replace('policy', 'procedure').replace('سياسة', 'إجراء')
            prompt += """\n\nIMPORTANT: This is a PROCEDURE document, not a policy. Generate a detailed step-by-step operational procedure with:
1. Purpose and scope
2. Roles and responsibilities (RACI matrix)
3. Prerequisites and required tools
4. Detailed step-by-step instructions with numbered steps
5. Decision flowchart points (if/then conditions)
6. Verification and validation steps
7. Exception handling procedures
8. Forms/templates/checklists to use
9. Record keeping requirements
10. Review and update frequency
Format all steps as numbered lists with clear action verbs. Include tables for RACI, tools, and checklists."""
