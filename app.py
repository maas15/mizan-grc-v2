...
⚠ قاعدة حاسمة — ضوابط خاصة بالسيناريو:
الضوابط الموصى بها يجب أن تكون مخصصة للتهديد والأصل المحددين أعلاه.
لا تكتب ضوابط عامة مثل "تفعيل MFA" أو "تحديث أنظمة الكشف" أو "تدريب التوعية" لكل سيناريو.

أمثلة للضوابط الصحيحة حسب السيناريو:
- تهديد داخلي لقاعدة بيانات ← تحليلات سلوك المستخدم (UEBA)، مراقبة نشاط قاعدة البيانات (DAM)، منع تسريب البيانات (DLP)
- فدية تستهدف الأجهزة ← EDR مع كشف سلوكي، نسخ احتياطية معزولة غير قابلة للتعديل، بوابة بريد مع حماية رمل
- تصيد يستهدف الموظفين ← بوابة أمن بريد مع DMARC/DKIM/SPF، محاكاة تصيد دورية، كشف اختراق البريد التجاري
- هجوم DDoS ← WAF مع تحديد معدل الطلبات، حماية CDN، خدمة تنظيف حركة المرور
...
⚠ CRITICAL — SCENARIO-SPECIFIC CONTROLS:
The recommended controls MUST be tailored to the SPECIFIC threat and asset described above.
Do NOT generate generic controls like "Enable MFA", "Update threat detection", "Security awareness training" for every risk scenario.

Examples of GOOD scenario-specific controls:
- Insider threat to database → UEBA, Database Activity Monitoring, DLP for database exports, Privileged Access Management
- Ransomware to endpoints → EDR with behavioral detection, Immutable air-gapped backups, Email gateway sandboxing
- Phishing targeting employees → Email security gateway with DMARC/DKIM/SPF, Phishing simulation campaigns, BEC detection rules
- DDoS against web services → WAF with rate limiting, CDN-based DDoS mitigation, Traffic scrubbing service
- Data breach of PII → Data classification + DLP, Encryption at rest and in transit, Database access monitoring
- Cloud misconfiguration → CSPM tool deployment, Cloud IAM hardening, Cloud workload protection
