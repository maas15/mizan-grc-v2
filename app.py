...
def init_db():
    ...
    try:
        ...
        pass
    try:
        cursor.execute('ALTER TABLE users ADD COLUMN token_limit INTEGER DEFAULT 50000')
    except:
        pass
    ...
    # ── Seed grc_frameworks from DOMAIN_FRAMEWORKS if table is empty ──
    existing_fw = cursor.execute('SELECT COUNT(*) FROM grc_frameworks').fetchone()[0]
    if existing_fw == 0:
        fw_rows = []
        sort_idx = 0
...
| 4 | [Risk] | High/Medium/Low | High/Medium/Low | [Action] |"""
        ...
        def identify_section(text, lang_code):
            ...
            if 'مؤشرات الأداء' in first_line:
                return 'kpis'
            if 'تقييم الثقة' in first_line or 'درجة الثقة' in first_line:
                return 'confidence'
            ...
            keyword_scores = {
                ...
                'confidence': [('confidence score', 3), ('confidence assessment', 3), ('overall confidence', 3), ('الثقة', 2), ('تقييم الثقة', 3), ('درجة الثقة', 3)]
            }
...
@app.route('/api/compliance/trend', methods=['GET'])
@login_required
def api_compliance_trend():
    """Retrieve compliance history for trend charts."""
    try:
        user_id = session['user_id']
        days = request.args.get('days', 90, type=int)
        history = get_compliance_trend(user_id, days)
        return jsonify({'success': True, 'trend': history})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
