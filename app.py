        ...
        def inject_kpi_assessment_guides(kpi_content, lang_code):
            ...
            if lang_code == 'ar':
                ...
                for num, kpi_name in kpi_list:
                    # Skip if this KPI already has a guide with table
                    _gp = f'دليل تقييم المؤشر رقم {num}'
                    if _gp in kpi_content:
                        _gi = kpi_content.find(_gp)
                        _gc = kpi_content[_gi:_gi+600]
                        if '| الخطوة |' in _gc or '| الإجراء |' in _gc or '|---' in _gc:
                            continue
                ...
                for num, kpi_name in kpi_list:
                    # Skip if this KPI already has a guide with table
                    _gp_en = f'KPI #{num} Assessment Guide'
                    if _gp_en in kpi_content:
                        _gi_en = kpi_content.find(_gp_en)
                        _gc_en = kpi_content[_gi_en:_gi_en+600]
                        if '| Step |' in _gc_en or '| Action |' in _gc_en or '|---' in _gc_en:
                            continue
