...
def domain_page(domain_name):
    ...
    return render_template('domain.html',
                          ...
                          awareness_modules=AWARENESS_MODULES.get(domain_code, {}).get('ar' if lang == 'ar' else 'en', []))
...
| # | Observation | Affected Control | Recommendation | Status |
|---|-------------|-----------------|----------------|--------|
| 1 | Finding | Control ID | Action | To Be Assessed |
...
⚠️ CRITICAL: For EACH High-Risk and Medium-Risk finding above, provide step-by-step implementation guidelines:

---
### Finding #1 Implementation Guide: [Finding Title]
...
| Phase | Step | Description | Owner | Deliverable |
|-------|------|-------------|-------|-------------|
| Planning | 1.1 | [Step description] | [Team] | [Output] |
| Implementation | 2.1 | [Step description] | [Team] | [Output] |
| Verification | 3.1 | [Step description] | [Team] | [Output] |
(6-10 steps per finding)
...
⚠️⚠️⚠️ FINAL CRITICAL REMINDER ⚠️⚠️⚠️
The "Detailed Implementation Guidelines" section is MANDATORY and MUST be included.
For EACH High-Risk and Medium-Risk finding, you MUST provide:
1. Step-by-step implementation table with Phase, Step, Description, Owner, Deliverable columns
2. 6-10 specific implementation steps
3. Evidence checklist for closure
Without these implementation guidelines, the audit report is INCOMPLETE and REJECTED."""
...
| # | الملاحظة | الضابط المتأثر | التوصية | الحالة |
|---|----------|---------------|---------|--------|
| 1 | الملاحظة | رمز الضابط | الإجراء | يحتاج تقييم |
...
⚠️ مهم جداً: لكل ملاحظة عالية ومتوسطة الخطورة، قدم دليل تنفيذ خطوة بخطوة:

---
### دليل تنفيذ الملاحظة رقم 1: [عنوان الملاحظة]
...
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | [وصف الخطوة] | [الفريق] | [المخرج] |
| التنفيذ | 2.1 | [وصف الخطوة] | [الفريق] | [المخرج] |
| التحقق | 3.1 | [وصف الخطوة] | [الفريق] | [المخرج] |
(6-10 خطوات لكل ملاحظة)
...
⚠️⚠️⚠️ تذكير نهائي مهم جداً ⚠️⚠️⚠️
قسم "دليل التنفيذ التفصيلي" إلزامي ويجب تضمينه.
لكل ملاحظة عالية ومتوسطة الخطورة، يجب تقديم:
1. جدول تنفيذ خطوة بخطوة بأعمدة: المرحلة، الخطوة، الوصف، المسؤول، المخرجات
2. 6-10 خطوات تنفيذ محددة
3. قائمة الأدلة المطلوبة للإغلاق
بدون أدلة التنفيذ هذه، يعتبر تقرير التدقيق غير مكتمل ومرفوض."""
