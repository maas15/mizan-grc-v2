...
def get_user_usage_by_domain(user_id, domain):
    ...
    usage = {
        ...
        'policies': conn.execute('SELECT COUNT(*) FROM policies WHERE user_id = ? AND domain = ? AND COALESCE(is_procedure, 0) = 0', (user_id, domain)).fetchone()[0],
        'procedures': conn.execute('SELECT COUNT(*) FROM policies WHERE user_id = ? AND domain = ? AND is_procedure = 1', (user_id, domain)).fetchone()[0],
...
def profile_page():
    ...
    for domain in en_domains:
        ...
        p_count = conn.execute('SELECT COUNT(*) FROM policies WHERE user_id = ? AND domain = ? AND COALESCE(is_procedure, 0) = 0', (session['user_id'], domain)).fetchone()[0]
        proc_count = conn.execute('SELECT COUNT(*) FROM policies WHERE user_id = ? AND domain = ? AND is_procedure = 1', (session['user_id'], domain)).fetchone()[0]
...
def domain_page(domain_name):
    ...
    grc_domain_relevance = {
        'cyber': ['grc_nca_frameworks', 'grc_iso27001', 'grc_nist_csf', 'grc_sama_framework', 'grc_incident_bcm', 'grc_fundamentals'],
        'data': ['grc_pdpl', 'grc_gdpr', 'grc_fundamentals'],
        'ai': ['grc_eu_regulations', 'grc_fundamentals'],
        'dt': ['grc_iso27001', 'grc_fundamentals'],
        'global': ['grc_iso27001', 'grc_gdpr', 'grc_eu_regulations', 'grc_fundamentals'],
        'erm': ['grc_coso_erm', 'grc_fundamentals']
    }
