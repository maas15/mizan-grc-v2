...
def generate_ai_content(prompt, language='en', task_type='generate'):
    ...
    try:
        if provider == 'anthropic':
            return _generate_anthropic(system_prompt, prompt, language)
        elif provider == 'openai':
            return _generate_openai(system_prompt, prompt, language)
        elif provider == 'google':
            return _generate_google(system_prompt, prompt, language)
        elif provider == 'groq':
            return _generate_groq(system_prompt, prompt, language)
    except Exception as e:
...
|---|--------|----------------|--------------|----------|--------|
| 1 | [اسم الفجوة] | [وصف شامل للفجوة وأسبابها] | [الأثر على المنظمة] | عالية/متوسطة | يحتاج تقييم |
(5-6 فجوات رئيسية مع "يحتاج تقييم" في عمود الحالة)
...
(4-5 gaps with "To Be Assessed" in Status column)
