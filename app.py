...
⚠️ مهم جداً: لكل فجوة محددة أعلاه، يجب تقديم دليل تنفيذ تفصيلي بالتنسيق التالي:
...
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | [وصف الخطوة] | [الجهة] | [المخرج] |
| التنفيذ | 2.1 | [وصف الخطوة] | [الجهة] | [المخرج] |
| المراقبة | 3.1 | [وصف الخطوة] | [الجهة] | [المخرج] |
(6-10 خطوات لكل فجوة)

**الأدلة المطلوبة للإغلاق:** ☐ [دليل 1] ☐ [دليل 2] ☐ [دليل 3]

---
(كرر نفس التنسيق لكل فجوة من الفجوات المحددة)
...
يجب أن تتضمن الوثيقة قسم "دليل التنفيذ لكل فجوة" في القسم 2 (تحليل الفجوات).
لكل فجوة محددة، يجب تقديم:
1. جدول تنفيذ تفصيلي بأعمدة: المرحلة، الخطوة، الوصف، المسؤول، المخرجات
2. 6-10 خطوات تنفيذ لكل فجوة
3. قائمة الأدلة المطلوبة للإغلاق
بدون أدلة التنفيذ هذه، تعتبر الوثيقة غير مكتملة ومرفوضة."""
        else:
...
⚠️ CRITICAL: For each gap identified above, provide a detailed step-by-step implementation guide in the following format:

---
#### Gap #1 Implementation Guide: [Gap Name]

**Step-by-Step Implementation:**
| Phase | Step | Description | Owner | Deliverable |
|-------|------|-------------|-------|-------------|
| Planning | 1.1 | [Step description] | [Team] | [Output] |
| Execution | 2.1 | [Step description] | [Team] | [Output] |
| Monitoring | 3.1 | [Step description] | [Team] | [Output] |
(6-10 steps per gap)

**Evidence Required for Closure:** ☐ [Evidence 1] ☐ [Evidence 2] ☐ [Evidence 3]

---
(Repeat same format for each identified gap)
...
You MUST include the "Implementation Guidelines for Each Gap" section in Section 2 (Gap Analysis).
For EACH gap identified, provide:
1. A detailed step-by-step implementation table with Phase, Step, Description, Owner, Deliverable columns
2. 6-10 implementation steps per gap
3. Evidence checklist for closure
Without these implementation guidelines, the document is INCOMPLETE and REJECTED."""
        ...
        def inject_implementation_guidelines(gaps_content, lang_code):
            ...
            for num, name in gaps_found:
                name = name.strip()
                if name and name.lower() not in ['gap', 'الفجوة', '#', 'description', 'الوصف', 'priority', 'الأولوية']:
                    gaps_list.append((num, name))
            
            if not gaps_list:
                return gaps_content
            
            # Generate guidelines template
            if lang_code == 'ar':
                guidelines = "\n\n---\n\n### دليل التنفيذ لكل فجوة:\n\n"
                for num, gap_name in gaps_list[:5]:  # Limit to first 5 gaps
                    guidelines += f"""---
...
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | تقييم الوضع الحالي وتحديد المتطلبات | فريق الأمن | تقرير التقييم |
| التخطيط | 1.2 | وضع خطة العمل والجدول الزمني | مدير المشروع | خطة معتمدة |
| التنفيذ | 2.1 | تخصيص الموارد والميزانية | الإدارة | موارد مخصصة |
| التنفيذ | 2.2 | تطبيق الإجراءات والضوابط | الفريق التقني | إجراءات مطبقة |
| التنفيذ | 2.3 | تدريب الفريق المعني | التدريب | فريق مدرب |
| التحقق | 3.1 | اختبار الفعالية | ضمان الجودة | تقرير الاختبار |
| التحقق | 3.2 | مراجعة وتوثيق النتائج | أمن المعلومات | وثائق الإغلاق |

**الأدلة المطلوبة للإغلاق:** ☐ تقرير التقييم ☐ خطة العمل ☐ سجلات التدريب ☐ تقرير الاختبار
...
| Phase | Step | Description | Owner | Deliverable |
|-------|------|-------------|-------|-------------|
| Planning | 1.1 | Assess current state and identify requirements | Security Team | Assessment report |
| Planning | 1.2 | Develop action plan and timeline | Project Manager | Approved plan |
| Execution | 2.1 | Allocate resources and budget | Management | Allocated resources |
| Execution | 2.2 | Implement procedures and controls | Technical Team | Implemented procedures |
| Execution | 2.3 | Train relevant staff | Training | Trained team |
| Verification | 3.1 | Test effectiveness | QA | Test report |
| Verification | 3.2 | Review and document results | InfoSec | Closure documentation |

**Evidence Required for Closure:** ☐ Assessment report ☐ Action plan ☐ Training records ☐ Test report
...
---
(Repeat for each High and Medium risk finding)
...
---
(كرر لكل ملاحظة عالية ومتوسطة الخطورة)
