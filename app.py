...
def api_generate_docx():
    ...
    try:
        ...
        def add_table_to_doc(doc, table_data, lang):
            ...
            # Smart column widths for 5+ column tables
            if num_cols >= 5:
                try:
                    header_row_text = [str(c).lower().strip() for c in table_data[0]]
                    # Detect special table types
                    has_phase = 'phase' in header_row_text or 'المرحلة' in header_row_text
                    has_step = 'step' in header_row_text or 'الخطوة' in header_row_text
                    has_action = 'action' in header_row_text or 'الإجراء' in header_row_text
                    
                    # Determine per-column relative widths
                    long_keywords = {'description', 'justification', 'mitigation', 'action', 'الوصف', 'المبرر', 'المعالجة', 'الإجراء'}
                    short_keywords = {'#', 'no', 'رقم', 'step', 'score', 'الدرجة'}
                    
                    rel_widths = []
                    for ci, h in enumerate(header_row_text):
                        if any(kw in h for kw in long_keywords):
                            rel_widths.append(4.0)
                        elif h in short_keywords or len(h) <= 2:
                            rel_widths.append(0.8)
                        elif has_phase and h in ('phase', 'المرحلة'):
                            rel_widths.append(2.0)
                        else:
                            rel_widths.append(1.5)
                    
                    total_rel = sum(rel_widths)
                    page_usable = Inches(5.5)  # ~8.5" - 2x1.5" margins
                    for ci, rw in enumerate(rel_widths):
                        table.columns[ci].width = int(page_usable * rw / total_rel)
                except Exception:
                    pass
        ...
        while i < len(lines):
            ...
            if line.startswith('|') and '|' in line[1:]:
                ...
                if table_data:
                    ...
                    # Check if this is an implementation guidelines table (has BOTH Phase AND Step)
                    header_row = [str(c).lower().strip() for c in table_data[0]] if table_data else []
                    is_implementation_table = (
                        ('phase' in header_row and 'step' in header_row) or
                        ('المرحلة' in header_row and 'الخطوة' in header_row)
                    )
                    # KPI Assessment tables: Step | Action | Tool/System | Owner | Output
                    is_kpi_assessment_table = (
                        col_count == 5 and
                        ('step' in header_row or 'الخطوة' in header_row) and
                        ('action' in header_row or 'الإجراء' in header_row) and
                        'phase' not in header_row and 'المرحلة' not in header_row
                    )
                    ...
                    if is_implementation_table and col_count == 5:
                        ...
                        # KPI Assessment table: Step, Action, Tool/System, Owner, Output
                        # Action column is the widest (long descriptive text)
                        if is_arabic:
                            col_widths = [
                                available_width * 0.14,  # المخرج (Output)
                                available_width * 0.14,  # المسؤول (Owner)
                                available_width * 0.16,  # الأداة (Tool)
                                available_width * 0.48,  # الإجراء (Action) - main content
                                available_width * 0.08,  # الخطوة (Step) - just numbers
                            ]
                        else:
                            col_widths = [
                                available_width * 0.07,  # Step - just numbers 1-4
                                available_width * 0.43,  # Action - long descriptive text
                                available_width * 0.18,  # Tool/System
                                available_width * 0.16,  # Owner
                                available_width * 0.16,  # Output
                            ]
                    elif col_count >= 7:
                        # Content-aware width distribution for dense tables
                        # Scan header text to find description/justification/long-text columns
                        long_keywords = {'description', 'justification', 'mitigation', 'الوصف', 'المبرر', 'المعالجة', 'action', 'الإجراء'}
                        short_keywords = {'#', 'no', 'رقم', 'score', 'الدرجة', 'likelihood', 'impact', 'الاحتمالية', 'الأثر', 'timeframe'}
                        
                        wide_cols = set()
                        narrow_cols = set()
                        for ci, h in enumerate(header_row):
                            h_lower = h.lower().strip()
                            if any(kw in h_lower for kw in long_keywords):
                                wide_cols.add(ci)
                            elif h_lower in short_keywords or (len(h_lower) <= 2):
                                narrow_cols.add(ci)
                        
                        if wide_cols or narrow_cols:
                            # Smart distribution: narrow=6%, wide=proportional share of remainder
                            narrow_pct = 0.06
                            normal_count = col_count - len(wide_cols) - len(narrow_cols)
                            wide_count = len(wide_cols) or 1
                            narrow_total = len(narrow_cols) * narrow_pct
                            normal_pct = 0.10
                            normal_total = normal_count * normal_pct
                            wide_each = max(0.12, (1.0 - narrow_total - normal_total) / wide_count)
                            
                            col_widths = []
                            for ci in range(col_count):
                                if ci in narrow_cols:
                                    col_widths.append(available_width * narrow_pct)
                                elif ci in wide_cols:
                                    col_widths.append(available_width * wide_each)
                                else:
                                    col_widths.append(available_width * normal_pct)
                            
                            # Normalize to available_width
                            total = sum(col_widths)
                            col_widths = [w * available_width / total for w in col_widths]
                        else:
                            # Fallback: narrow first col, rest equal
                            narrow_col = 0.35 * inch
