...
TRANSLATIONS = {
    "en": {
        ...
        "step1_desc": "Choose from 5 specialized GRC domains",
    ...
    "ar": {
        ...
        "feature_domains": "5 مجالات للحوكمة",
        ...
        "step1_desc": "اختر من 5 مجالات متخصصة في الحوكمة",
...
def api_modify_policy():
    """Modify a policy based on review findings."""
    try:
        data = request.json
        ...
        if len(review_content) > 5000:
            review_content = review_content[:5000] + "..."
...
Write the complete modified policy (not just the changes):"""
        ...
        modified_policy = generate_ai_content(prompt, lang)
        ...
        if latest_policy:
            conn.execute('UPDATE policies SET content = ? WHERE id = ?',
                        (modified_policy, latest_policy['id']))
            conn.commit()
        conn.close()
        ...
        print(f"Policy modification error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def api_generate_docx():
    ...
    try:
        ...
        # ---- Add document title ----
        add_rtl_heading(filename.replace('_', ' ').title(), 0)
        ...
        while i < len(lines):
            ...
            # Bullet points
                ...
                raw_text = line[2:]
                if is_arabic:
                    # Arabic: text then bullet on right, no built-in bullet style
                    p = doc.add_paragraph()
        ...
        while i < len(lines):
            ...
            if line.startswith('|') and '|' in line[1:]:
                ...
                if table_data:
                    ...
                    # For Arabic: columns are reversed, so narrow # col is on the right (last)
                    if col_count >= 7:
                        narrow_col = 0.35 * inch
                        remaining = doc.width - narrow_col
                        ...
                        if is_arabic:
                            col_widths = [2.5*inch, 2.5*inch, 0.5*inch]
                        else:
                            col_widths = [0.5*inch, 2.5*inch, 2.5*inch]
                    elif col_count == 2:
                        col_widths = [doc.width - 2*inch, 2*inch] if is_arabic else [2*inch, doc.width - 2*inch]
                    else:
                    ...
                    for row_idx, row in enumerate(table_data):
                        ...
                        for cell in row:
                            cell_text = str(cell)
                            # Process Arabic: reshape and bidi
                            if is_arabic and cell_text:
                                cell_text = process_arabic(cell_text)
            ...
            if line.startswith('#### '):
                raw_text = line[5:]
                text = process_arabic(raw_text) if is_arabic else raw_text
                ...
                text = process_arabic(raw_text) if is_arabic else raw_text
                if is_arabic:
                    # Arabic: text first, then bullet on right
                    bullet_text = text + ' •'
                else:
                    bullet_text = '• ' + text
                story.append(Paragraph(bullet_text, bullet_style))
            elif is_arabic and len(line) > 2 and line[0].isdigit() and (line[1] == '.' or (len(line) > 2 and line[1].isdigit() and line[2] == '.')):
                # Arabic numbered list: "1. الغرض" → "الغرض .1"
                import re as re_mod
                num_match = re_mod.match(r'^(\d+)\.\s*(.+)', line)
                if num_match:
                    num = num_match.group(1)
                    raw_text = num_match.group(2)
                    text = process_arabic(raw_text)
                    numbered_text = text + ' .' + num
                    story.append(Paragraph(numbered_text, numbered_style))
                else:
                    text = process_arabic(line)
                    story.append(Paragraph(text, normal_style))
            elif not is_arabic and len(line) > 2 and line[0].isdigit() and '.' in line[:3]:
                # English numbered list: keep as is
                import re as re_mod
                num_match = re_mod.match(r'^(\d+)\.\s*(.+)', line)
                if num_match:
                ...
                for idx, part in enumerate(parts):
                    if idx % 2 == 1:  # Bold text
                        p = process_arabic(part) if is_arabic else part
