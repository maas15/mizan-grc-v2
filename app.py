...
|---|--------|-------|----------|--------|
| 1 | {"فجوة الهيكل التنظيمي" if has_no_structure else "فجوة السياسات والحوكمة"} | {"عدم وجود إدارة مختصة بالأمن السيبراني - هذا الأساس لتطبيق جميع الضوابط الأخرى" if has_no_structure else f"الحاجة لتحديث السياسات لتتوافق مع متطلبات {fw}"} | {"حرجة" if has_no_structure else "عالية"} | يحتاج تقييم |
...
|---|-----|-------------|----------|--------|
| 1 | {"Organizational Structure Gap" if has_no_structure else "Policy & Governance Gap"} | {"No dedicated cybersecurity department - foundation for all other controls" if has_no_structure else f"Need to update policies to meet {fw} requirements"} | {"Critical" if has_no_structure else "High"} | To Be Assessed |
...
def generate_audit_simulation(language='en', framework='NCA ECC', policy_name=None):
    ...
    AUDIT_FINDINGS_AR = {
        'البريد الإلكتروني': [
            ('عدم تفعيل تشفير البريد الإلكتروني للرسائل الحساسة', 'ضابط حماية البيانات أثناء النقل', 'تفعيل TLS إجباري وتوفير خيار تشفير الرسائل'),
            ('عدم وجود آلية فعالة لتصفية البريد الاحتيالي', 'ضابط الحماية من البرمجيات الخبيثة', 'تفعيل بوابة حماية البريد مع فحص الروابط والمرفقات'),
        ],
        'أمن الشبكات': [
            ('عدم تقسيم الشبكة إلى مناطق أمنية منفصلة', 'ضابط أمن الشبكات', 'تطبيق تقسيم الشبكة (Network Segmentation) وفصل الأنظمة الحساسة'),
            ('عدم تحديث البرامج الثابتة لأجهزة الشبكة', 'ضابط إدارة الثغرات', 'إنشاء جدول تحديث دوري لجميع أجهزة الشبكة'),
        ],
        'الاستخدام المقبول': [
            ('عدم وجود آلية مراقبة لاستخدام الإنترنت', 'ضابط المراقبة والتسجيل', 'تفعيل نظام تصفية المحتوى ومراقبة الاستخدام'),
    ...
    AUDIT_FINDINGS_EN = {
        'Email': [
            ('Email encryption not enabled for sensitive communications', 'Data in Transit Protection Control', 'Enable mandatory TLS and provide message encryption options'),
            ('No effective phishing email filtering mechanism', 'Malware Protection Control', 'Deploy email security gateway with URL and attachment scanning'),
        ],
        'Network Security': [
            ('Network not segmented into separate security zones', 'Network Security Control', 'Implement network segmentation and isolate sensitive systems'),
            ('Network device firmware not regularly updated', 'Vulnerability Management Control', 'Establish regular update schedule for all network devices'),
        ],
        'Acceptable Use': [
            ('No internet usage monitoring mechanism in place', 'Monitoring and Logging Control', 'Deploy content filtering and usage monitoring system'),
...
| التخطيط | 1.3 | إعداد خطة التنفيذ والميزانية | الإدارة المالية | خطة معتمدة |
| التنفيذ | 2.1 | تجهيز البيئة التقنية | فريق البنية التحتية | بيئة جاهزة |
| التنفيذ | 2.2 | تطبيق الحل وتكوينه | فريق الأمن | نظام مُفعّل |
| التنفيذ | 2.3 | اختبار شامل في بيئة تجريبية | ضمان الجودة | تقرير الاختبار |
| النشر | 3.1 | تدريب الموظفين المعنيين | التدريب | سجلات التدريب |
| النشر | 3.2 | التشغيل التدريجي والمراقبة | فريق الأمن | تقارير المراقبة |
...
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | مراجعة الوضع الحالي وتحديد الفجوات | فريق الأمن | تقرير الفجوات |
| التخطيط | 1.2 | تحديد المتطلبات وفق {fw} | الامتثال | قائمة المتطلبات |
| التنفيذ | 2.1 | تطوير/تحديث السياسة أو الإجراء | أمن المعلومات | وثيقة محدثة |
| التنفيذ | 2.2 | مراجعة واعتماد من الإدارة المعنية | الإدارة التنفيذية | وثيقة معتمدة |
| النشر | 3.1 | تواصل مع الأطراف المعنية وتدريب | التدريب | سجلات التدريب |
| المراقبة | 4.1 | متابعة التطبيق والتحقق من الالتزام | فريق الأمن | تقرير المتابعة |

**الأدلة المطلوبة للإغلاق:**
- [ ] الوثيقة المحدثة والمعتمدة
- [ ] سجلات التواصل والتدريب
- [ ] تقرير المتابعة
...
|---|----------|---------------|---------|--------|
| 3 | نقص في التوثيق المتعلق بـ{pn} | ضابط التوثيق والسياسات | تحديث وتوثيق جميع الإجراءات | يحتاج تقييم |
| 4 | تأخر في برامج التوعية المتعلقة بـ{pn} | ضابط التوعية والتدريب | إطلاق برنامج توعية مخصص | يحتاج تقييم |

### نتائج منخفضة الخطورة
| # | الملاحظة | الضابط المتأثر | التوصية | الحالة |
|---|----------|---------------|---------|--------|
| 5 | عدم وجود مراجعة دورية لسياسة {pn} | ضابط المراجعة الدورية | جدولة مراجعة سنوية | يحتاج تقييم |
...
| 3 | تحديث التوثيق | أمن المعلومات | خلال 60 يوم | متوسطة |
| 4 | إطلاق برنامج التوعية | الموارد البشرية | خلال 60 يوم | متوسطة |
| 5 | جدولة المراجعة الدورية | أمن المعلومات | خلال 90 يوم | منخفضة |
...
| Planning | 1.3 | Prepare implementation plan and budget | Finance | Approved plan |
| Implementation | 2.1 | Prepare technical environment | Infrastructure Team | Ready environment |
| Implementation | 2.2 | Deploy and configure solution | Security Team | Activated system |
| Implementation | 2.3 | Comprehensive testing in test environment | QA | Test report |
| Deployment | 3.1 | Train relevant personnel | Training | Training records |
| Deployment | 3.2 | Gradual rollout and monitoring | Security Team | Monitoring reports |

**Evidence Required:** ☐ Implementation report ☐ Configuration screenshots ☐ Training records ☐ Test report
...
| Phase | Step | Description | Owner | Deliverable |
|-------|------|-------------|-------|-------------|
| Planning | 1.1 | Review current state and identify gaps | Security Team | Gap report |
| Planning | 1.2 | Identify {fw} requirements | Compliance | Requirements list |
| Implementation | 2.1 | Develop/update policy or procedure | InfoSec | Updated document |
| Implementation | 2.2 | Review and approval by management | Executive Management | Approved document |
| Deployment | 3.1 | Communicate to stakeholders and train | Training | Training records |
| Monitoring | 4.1 | Monitor implementation and verify compliance | Security Team | Monitoring report |

**Evidence Required:** ☐ Updated and approved document ☐ Communication and training records ☐ Monitoring report
...
|---|-------------|-----------------|----------------|--------|
| 3 | Insufficient documentation related to {pn} | Documentation & Policy Control | Update and document all procedures | To Be Assessed |
| 4 | Delayed awareness programs related to {pn} | Awareness & Training Control | Launch dedicated awareness program | To Be Assessed |

### Low-Risk Findings
| # | Observation | Affected Control | Recommendation | Status |
|---|-------------|-----------------|----------------|--------|
| 5 | No periodic review of {pn} policy | Periodic Review Control | Schedule annual review | To Be Assessed |
...
| 3 | Update documentation | InfoSec | Within 60 days | Medium |
| 4 | Launch awareness program | HR | Within 60 days | Medium |
| 5 | Schedule periodic review | InfoSec | Within 90 days | Low |
...
| **الاحتمالية الإجمالية** | **يُحدد بعد التقييم** |
...
|----------|------------|------------|
| مستوى الخطر | يُحدد بعد التقييم | يُحدد بعد تطبيق الضوابط |
| الاحتمالية | يُحدد بعد التقييم | يُحدد بعد تطبيق الضوابط |
| الأثر المالي | يُحدد بعد التقييم | يُحدد بعد تطبيق الضوابط |
...
| **Overall Likelihood** | **To be assessed** |
...
|----------|-----------------|----------------|
| Risk Level | To be assessed | To be assessed after controls |
| Likelihood | To be assessed | To be assessed after controls |
| Financial Impact | To be assessed | To be assessed after controls |
...
CRITICAL INSTRUCTION - SELECTED FRAMEWORKS ONLY:
...
6. Do NOT echo any instruction labels or section headers from this prompt into the output. The output must contain only the strategy document content.
...
{"CRITICAL: The organization currently has NO dedicated cybersecurity department. Objective #1 MUST be establishing a cybersecurity department. Pillar #1 MUST detail the proposed organizational structure (CISO, Security Operations team, Governance team, Incident Response team) with reporting lines and authority. This is the foundation for all other objectives." if org_structure and ('no' in org_structure.lower() or 'none' in org_structure.lower()) else ""}
...
This audit is specifically focused on "{audit_topic}". ALL findings, observations, and recommendations must be directly related to {audit_topic} and its compliance with {framework} controls.
Do NOT produce generic findings unrelated to {audit_topic}."""
...
CRITICAL INSTRUCTION - SELECTED FRAMEWORK ONLY:
...
8. Do NOT include generic "Audit Scope" or "Audit Methodology" sections - this audit assesses compliance against {framework} controls, so jump directly to findings
9. Do NOT include any section listing scope bullet points or methodology steps - the reader already knows the audit approach
...
## Executive Summary
Brief overview of audit results - overall compliance status {"of " + audit_topic + " " if audit_topic else ""}against {framework} requirements
...
## الملخص التنفيذي
يقدم هذا التقرير نتائج تدقيق {"موضوع " + audit_topic + " وفق " if audit_topic else "الامتثال لـ"}إطار {framework}. تم تحديد عدة ملاحظات تتطلب معالجة لتحسين مستوى الامتثال. سيتم تحديد نسبة الامتثال الفعلية بعد استكمال التقييم الميداني.
