...
def api_generate_strategy():
    ...
    try:
        ...
        if lang == 'ar':
            ...
            prompt = f"""أنت خبير استراتيجي في مجال {domain_desc} ذو خبرة واسعة في وضع الاستراتيجيات وفق الأطر الدولية.

أنشئ وثيقة استراتيجية احترافية ومتكاملة لمجال **{domain_desc}** وفق إطار {frameworks_list}.
...
الهيكل التنظيمي: {org_structure}
التقنيات المطبقة: {tech_list}
مستوى النضج: {maturity}
التحديات: {data.get('challenges', 'غير محدد')}

قواعد صارمة:
- المحتوى يجب أن يكون خاصاً بمجال {domain} فقط ومرتبطاً بإطار {frameworks_list} حصراً
- لا تذكر أي إطار آخر غير {frameworks_list}
- لا تستخدم أي نسب مئوية محددة للقيم الحالية - استخدم "يُحدد بعد التقييم"
- لا تستخدم أي أسماء أشخاص
- لا تكتب أي تعليمات أو تنبيهات أو ملاحظات موجهة في المخرجات النهائية
- استخدم [SECTION] بين الأقسام الستة

اكتب 6 أقسام بالتنسيق التالي:

## 1. الرؤية والأهداف الاستراتيجية

**الرؤية الاستراتيجية:**
(اكتب فقرتين على الأقل تصفان الرؤية الاستراتيجية بشكل مفصل وطموح خاص بمجال {domain}، مع ربطها بأهداف المنظمة ورؤيتها المستقبلية وإطار {frameworks_list})

**الرسالة:**
(اكتب رسالة واضحة تحدد الغرض الأساسي من هذه الاستراتيجية)

### الأهداف الاستراتيجية:
| # | الهدف الاستراتيجي | المؤشر المستهدف | المبرر | الإطار الزمني |
|---|-------------------|-----------------|--------|---------------|
(5 أهداف في جدول بهذا التنسيق مع مبررات مرتبطة بإطار {frameworks_list})

[SECTION]

## 2. تحليل الفجوات وأدلة التنفيذ

### الفجوات المحددة:
| # | الفجوة | الوصف التفصيلي | الأولوية | الأثر | الحالة |
|---|--------|----------------|---------|-------|--------|
{gaps_table}

(مباشرة أسفل الجدول، اكتب دليل تنفيذ مفصل لكل فجوة - يجب أن يكون كل دليل بمحتوى مختلف تماماً ومخصص للفجوة المحددة)

---
#### دليل تنفيذ الفجوة رقم 1: (اسم الفجوة الأولى من الجدول)
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | (خطوة تفصيلية خاصة بهذه الفجوة) | (الفريق) | (المخرج) |
| التخطيط | 1.2 | (خطوة تفصيلية خاصة بهذه الفجوة) | (الفريق) | (المخرج) |
| التنفيذ | 2.1 | (خطوة تفصيلية خاصة بهذه الفجوة) | (الفريق) | (المخرج) |
| التنفيذ | 2.2 | (خطوة تفصيلية خاصة بهذه الفجوة) | (الفريق) | (المخرج) |
| التحقق | 3.1 | (خطوة تفصيلية خاصة بهذه الفجوة) | (الفريق) | (المخرج) |
**الأدلة المطلوبة:** ☐ (دليل 1) ☐ (دليل 2) ☐ (دليل 3)

(كرر نفس التنسيق بالضبط للفجوات رقم 2 و3 و4 و5 بمحتوى مختلف تماماً يخص كل فجوة)

[SECTION]

## 3. الركائز الاستراتيجية

(اكتب 4 ركائز، كل ركيزة تحتوي على وصف فقرة + 3 مبادرات تفصيلية)

### الركيزة 1: (الاسم)
**الوصف:** (فقرة تشرح الركيزة وأهميتها)
• مبادرة تفصيلية 1
• مبادرة تفصيلية 2
• مبادرة تفصيلية 3
...
## 5. مؤشرات الأداء الرئيسية وأدلة القياس

### مؤشرات الأداء:
| # | المؤشر | الوصف | القيمة الحالية | القيمة المستهدفة | المبرر | الإطار الزمني | مصدر البيانات |
|---|--------|-------|----------------|------------------|--------|---------------|---------------|
(8 مؤشرات KPI خاصة بمجال {domain} - القيمة الحالية دائماً "يُحدد بعد التقييم")

### أدلة قياس مؤشرات الأداء:

(لكل مؤشر من المؤشرات الثمانية أعلاه، اكتب دليل قياس مفصل بنفس التنسيق التالي بمحتوى مخصص لكل مؤشر)

---
#### دليل قياس المؤشر رقم 1: (اسم المؤشر الأول)
| الخطوة | الإجراء | المسؤول | المخرج |
|--------|---------|---------|--------|
| 1 | تحديد نطاق القياس ومعايير التقييم | (الفريق) | وثيقة النطاق |
| 2 | تحديد مصادر البيانات والأدوات اللازمة | (الفريق) | قائمة المصادر |
| 3 | جمع البيانات الأساسية من المصادر | (الفريق) | البيانات الخام |
| 4 | التحقق من دقة البيانات ومراجعتها | (الفريق) | بيانات موثقة |
| 5 | احتساب القيمة الحالية وتوثيق المنهجية | (الفريق) | قيمة المؤشر |

(كرر لجميع المؤشرات الثمانية بمحتوى مخصص لكل مؤشر)

[SECTION]

## 6. تقييم الجاهزية والمخاطر

**درجة الثقة:** يُحدد بعد التقييم

**مبرر التقييم:**
(فقرة تفصيلية تشرح أساس تقييم الجاهزية مع ذكر العوامل المؤثرة)
...
CRITICAL: This strategy must be entirely focused on {domain}. Do NOT mix with other domains.
...
IMPORTANT: Provide a SEPARATE, COMPLETE implementation guide for EACH gap identified above. Do NOT write "repeat" or "same as above" - write ACTUAL steps for each gap.
...
- استخدم شرطة (-) للقوائم
- استخدم رموز الضوابط الفعلية من إطار {framework}
