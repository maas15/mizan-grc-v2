...
def api_podcast_generate():
    ...
    try:
        ...
        if lang == 'ar':
            prompt = f"""أنت كاتب سيناريو بودكاست محترف. اكتب حواراً طبيعياً وحيوياً بين خبيرين:
- د. خالد: مستشار GRC أول بخبرة 15 سنة، أسلوبه تحليلي واستراتيجي
- م. نورة: مديرة امتثال بخبرة 10 سنوات، أسلوبها عملي ومباشر

الموضوع: {topic_name} في مجال {domain}
الأطر المرجعية: {fw_text}

أجب بصيغة JSON فقط بدون أي نص إضافي أو backticks:
{{"dialogue": [
  {{"speaker": "A", "text": "مرحباً بكم في بودكاست ميزان..."}},
  {{"speaker": "B", "text": "أهلاً..."}},
...
- اذكر أطر ومعايير محددة من القائمة أعلاه
- اجعل الحوار يتضمن نقاشاً وتبادل آراء وليس مجرد سرد
- كل سطر 2-4 جمل — مناسب للقراءة الصوتية
- لا تستخدم رموز تعبيرية أو نجوم أو أقواس"""
        else:
            prompt = f"""You are a professional podcast script writer. Write a natural, engaging dialogue between two experts:
- Dr. Sarah: Senior GRC Strategist with 15 years experience, analytical and strategic style
- James: Compliance Director with 10 years experience, practical and direct style
...
{{"dialogue": [
  {{"speaker": "A", "text": "Welcome to the Mizan GRC Podcast..."}},
...
- Each line should be 2-4 sentences — suitable for voice reading
- Make it sound like a real podcast conversation, not a lecture
