        ...
        def process_arabic(text, font_name_for_wrap=None, font_size_for_wrap=11, extra_indent=0):
            ...
            if is_arabic and text:
                try:
                    ...
                    # Allow ~5% overflow to prevent orphaning short words like في، من، أو
                    overflow_tolerance = wrap_width * 0.05
                    ...
                    for word in words:
                        ...
                        if test_width > wrap_width and current_line_words:
                            # Check if word is short enough to keep on this line
                            # (prevents orphaning prepositions and short words)
                            if word_width < space_width * 6 and test_width <= wrap_width + overflow_tolerance:
                                # Short word barely overflows — keep it on this line
            ...
            if is_arabic and text:
                try:
                    ...
                    for word in words:
                        ...
                        test_width = current_width + word_width + (space_width if current_line_words else 0)
                        overflow_t = cell_width * 0.05
                        
                        if test_width > cell_width and current_line_words:
                            if word_width < space_width * 6 and test_width <= cell_width + overflow_t:
                                current_line_words.append(word)
                                current_width = test_width
                                continue
                            line_text = ' '.join(current_line_words)
