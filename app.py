...
اكتب 6 أقسام منفصلة خاصة بمجال {domain}. استخدم [SECTION] كفاصل بين كل قسم.
...
Without separate implementation guides for each gap, the document is INCOMPLETE and REJECTED."""
        ...
        if '[SECTION]' in content:
            ...
            # Try to split by ## X. pattern (section headers)
            # This is more reliable than [SECTION] markers
            section_split_pattern = r'(?=## [1-6]\.)'
            parts = re.split(section_split_pattern, content)
            parts = [p for p in parts if p.strip()]
            print(f"DEBUG: Split by ## X. pattern, got {len(parts)} parts", flush=True)
            
            if len(parts) < 4:
                # Try splitting by Arabic section numbers
                section_split_pattern_ar = r'(?=## \d+\.\s+(?:الرؤية|تحليل|الركائز|خارطة|مؤشرات|تقييم))'
                parts = re.split(section_split_pattern_ar, content)
                parts = [p for p in parts if p.strip()]
                print(f"DEBUG: Split by Arabic section pattern, got {len(parts)} parts", flush=True)
        
        # If still not enough parts, try splitting by numbered headers without ##
        if len(parts) < 4:
            section_split_pattern = r'(?=^[1-6]\.\s+)'
            parts = re.split(section_split_pattern, content, flags=re.MULTILINE)
            parts = [p for p in parts if p.strip()]
        ...
        # CRITICAL FIX: Ensure all 6 sections are filled
        section_order = ['vision', 'gaps', 'pillars', 'roadmap', 'kpis', 'confidence']
        
        # First, try order-based assignment for any parts that weren't identified
        for i, part in enumerate(parts[:6]):
            if i < len(section_order):
                section_name = section_order[i]
                # Only assign if section is still empty
...
6. الجداول يجب أن تكون بتنسيق Markdown الصحيح مع | في بداية ونهاية كل صف
7. القوائم المرقمة تبدأ بالرقم ثم النقطة (1. 2. 3.)
8. القوائم النقطية تبدأ بـ • ثم النص
...
## منهجية التدقيق
1. مراجعة الوثائق والسياسات
2. مقابلات مع الموظفين الرئيسيين
3. تقييم البنية التحتية التقنية
4. فحص الأمان الفيزيائي
5. تحليل البيانات وتقارير الامتثال
...
| # | الملاحظة | الضابط المتأثر | التوصية | الحالة |
|---|----------|---------------|---------|--------|
| 1 | [الملاحظة الأولى عالية الخطورة] | [رمز الضابط] | [الإجراء] | يحتاج تقييم |
| 2 | [الملاحظة الثانية عالية الخطورة] | [رمز الضابط] | [الإجراء] | يحتاج تقييم |

### نتائج متوسطة الخطورة
| # | الملاحظة | الضابط المتأثر | التوصية | الحالة |
|---|----------|---------------|---------|--------|
| 1 | [الملاحظة الأولى متوسطة الخطورة] | [رمز الضابط] | [الإجراء] | يحتاج تقييم |
| 2 | [الملاحظة الثانية متوسطة الخطورة] | [رمز الضابط] | [الإجراء] | يحتاج تقييم |

### نتائج منخفضة الخطورة
| # | الملاحظة | الضابط المتأثر | التوصية | الحالة |
|---|----------|---------------|---------|--------|
| 1 | [الملاحظة الأولى منخفضة الخطورة] | [رمز الضابط] | [الإجراء] | يحتاج تقييم |
| 2 | [الملاحظة الثانية منخفضة الخطورة] | [رمز الضابط] | [الإجراء] | يحتاج تقييم |

## أدلة التنفيذ التفصيلية

---
### دليل تنفيذ الملاحظة عالية الخطورة رقم 1: [عنوان الملاحظة الأولى]
**الضابط المتأثر:** [رمز الضابط]
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
| التخطيط | 1.2 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
| التنفيذ | 2.1 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
| التنفيذ | 2.2 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
| التحقق | 3.1 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
**الأدلة المطلوبة:** ☐ [دليل 1] ☐ [دليل 2] ☐ [دليل 3]

---
### دليل تنفيذ الملاحظة عالية الخطورة رقم 2: [عنوان الملاحظة الثانية]
**الضابط المتأثر:** [رمز الضابط]
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
...
---
### دليل تنفيذ الملاحظة متوسطة الخطورة رقم 1: [عنوان الملاحظة المتوسطة الأولى]
**الضابط المتأثر:** [رمز الضابط]
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
...
---
### دليل تنفيذ الملاحظة متوسطة الخطورة رقم 2: [عنوان الملاحظة المتوسطة الثانية]
**الضابط المتأثر:** [رمز الضابط]
| المرحلة | الخطوة | الوصف | المسؤول | المخرجات |
|---------|--------|-------|---------|----------|
| التخطيط | 1.1 | [خطوة محددة لهذه الملاحظة] | [الفريق] | [المخرج] |
...
| # | الإجراء | المسؤول | الموعد النهائي | الأولوية |
|---|--------|---------|---------------|----------|
| 1 | الإجراء | الفريق | خلال X يوم | عالية/متوسطة/منخفضة |
...
| العامل | الوزن | الدرجة | المبرر |
|--------|-------|--------|--------|
| جودة الأدلة | 25% | [X/10] | [تقييم جودة أدلة التدقيق] |
| تغطية اختبار الضوابط | 25% | [X/10] | [مدى شمولية الاختبار] |
| مراجعة التوثيق | 20% | [X/10] | [جودة الوثائق المراجعة] |
| موثوقية المقابلات | 15% | [X/10] | [الثقة في ردود المقابلات] |
| كفاية العينة | 15% | [X/10] | [كفاية حجم العينة] |

**تفسير مستوى الثقة:**
• 90-100%: ثقة عالية - النتائج مدعومة بشكل جيد
• 70-89%: ثقة متوسطة - النتائج على الأرجح دقيقة
• 50-69%: ثقة محدودة - يوصى بتحقق إضافي
• أقل من 50%: ثقة منخفضة - يوجد عدم يقين كبير
