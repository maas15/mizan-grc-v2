...
⚠️⚠️⚠️ قيد الأطر التنظيمية - مهم جداً ⚠️⚠️⚠️
المستخدم اختار هذه الأطر التنظيمية المحددة: {frameworks_list}
يجب أن تشير فقط وتستشهد بالأطر المدرجة أعلاه.
لا تذكر ISO أو NIST أو COBIT أو أي إطار آخر ما لم يكن مدرجاً صراحة في الأطر المختارة.
إذا اختار المستخدم "NCA ECC"، لا تذكر "ISO 27001" ما لم يتم اختيار ISO 27001 أيضاً.
هذا ضروري لدقة الوثيقة وأغراض الامتثال.
...
5. ⚠️ لا تفترض أي قيم أو نسب أو درجات حالية للمنظمة. القيم الحالية يجب أن تكون "يُحدد من قبل المنظمة" أو "يتطلب تقييم ميداني". فقط القيم المستهدفة يمكن تحديدها مع تقديم مبرر واضح يستند إلى الأطر المختارة أعلاه.
6. كل نسبة مئوية أو رقم أو تقييم يجب أن يكون مدعوماً بمرجع واضح من الأطر المختارة (وليس أطر عشوائية).
...
- **الأطر التنظيمية المختارة (استخدم هذه فقط): {frameworks_list}**
...
⚠️⚠️⚠️ تعليمات مهمة جداً للجداول ⚠️⚠️⚠️
- يجب كتابة الجداول بتنسيق Markdown الصحيح
- كل صف يبدأ وينتهي بـ |
- صف الفاصل يجب أن يحتوي على --- بين كل عمود
- مثال صحيح:
...
Without separate implementation guides for each gap, the document is INCOMPLETE and REJECTED."""
        ...
        def identify_section(text, lang_code):
            ...
            if '## 1.' in first_line or 'الرؤية' in first_line or ('1.' in first_line and 'الأهداف' in text[:200]):
                return 'vision'
            if '## 2.' in first_line or 'تحليل الفجوات' in first_line or ('2.' in first_line and 'الفجوات' in text[:200]):
                return 'gaps'
            if '## 3.' in first_line or 'الركائز' in first_line or ('3.' in first_line and 'الركائز' in text[:200]):
                return 'pillars'
            if '## 4.' in first_line or 'خارطة' in first_line or 'خارطة الطريق' in text[:200] or ('4.' in first_line and 'المرحلة' in text[:300]):
                return 'roadmap'
            if '## 5.' in first_line or 'مؤشرات' in first_line or 'مؤشرات الأداء' in text[:200] or ('5.' in first_line and 'مؤشر' in text[:200]):
                return 'kpis'
            if '## 6.' in first_line or 'تقييم الثقة' in first_line or 'درجة الثقة' in text[:200] or ('6.' in first_line and 'الثقة' in text[:200]):
                return 'confidence'
        ...
        def inject_implementation_guidelines(gaps_content, lang_code):
            ...
            has_guidelines = (
                'Implementation Guide' in gaps_content or 
                'Step-by-Step' in gaps_content or
                'دليل تنفيذ' in gaps_content or
                'الخطوات التفصيلية' in gaps_content
            )
            
            if has_guidelines:
                return gaps_content  # Already has guidelines
        ...
        # If we couldn't identify all sections, fall back to order-based assignment
        # This ensures all 6 sections are filled even if identification partially fails
        section_order = ['vision', 'gaps', 'pillars', 'roadmap', 'kpis', 'confidence']
        
        if len(assigned) < 6 and len(parts) >= 6:
            print(f"DEBUG: Some sections missing, using order-based assignment for remaining", flush=True)
            for i, part in enumerate(parts[:6]):
                if i < len(section_order):
                    ...
                    if not sections.get(section_name) or sections[section_name].strip() == '':
                        ...
                        print(f"DEBUG: Assigned part {i} to {section_name} (order-based fallback)", flush=True)
        elif len(assigned) < 3:
            print(f"DEBUG: Falling back to full order-based assignment (only {len(assigned)} sections identified)", flush=True)
            for i, part in enumerate(parts[:6]):
                if i < len(section_order):
