...
def api_generate_strategy():
    ...
    try:
        ...
        if lang == 'ar':
            ...
            prompt = f"""أنت مستشار خبير في الحوكمة والمخاطر والامتثال (GRC). أنشئ وثيقة استراتيجية شاملة ومفصلة.

⚠️⚠️⚠️ معلومات المشروع ⚠️⚠️⚠️
المجال: **{domain}**
الأطر التنظيمية المختارة: **{frameworks_list}**
المنظمة: {data.get('org_name', 'المنظمة')}
القطاع: {data.get('sector', 'حكومي')}
الحجم: {data.get('size', 'متوسط')}
الميزانية: {data.get('budget', 'غير محدد')}
مستوى النضج: {maturity}
التحديات: {data.get('challenges', 'غير محدد')}

⚠️⚠️⚠️ تعليمات إلزامية ⚠️⚠️⚠️
1. استخدم فقط الأطر المختارة ({frameworks_list}) - لا تذكر أي إطار آخر
2. لا تستخدم أرقام أو نسب أو قيم محددة مسبقاً
3. جميع القيم الحالية = "يُحدد بعد التقييم الميداني"
4. جميع القيم المستهدفة = "تُحدد بناءً على التقييم"
5. لا تستخدم تواريخ محددة - استخدم "خلال 6 أشهر" أو "السنة الأولى"
6. ⚠️ ضع [SECTION] بين كل قسم
...
**الرؤية الاستراتيجية:**
اكتب فقرة تصف رؤية المنظمة في مجال {domain} مرتبطة بإطار {frameworks_list}

### الأهداف الاستراتيجية:
| # | الهدف | المؤشر | الإطار الزمني |
|---|-------|--------|---------------|
اكتب 5-6 أهداف استراتيجية محددة لمجال {domain}
...
| # | الفجوة | الوصف | الأولوية | الحالة |
|---|--------|-------|---------|--------|
اكتب 5 فجوات حقيقية متعلقة بـ {domain} وإطار {frameworks_list}

### أدلة التنفيذ:
لكل فجوة، اكتب جدول خطوات التنفيذ:
...
اكتب 4 ركائز استراتيجية لمجال {domain}:

### الركيزة 1: (عنوان الركيزة)
**الهدف:** (وصف الهدف)
• (مبادرة 1)
• (مبادرة 2)
• (مبادرة 3)
...
|---|--------|---------|--------|--------|
اكتب 4 أنشطة للمرحلة الأولى

### المرحلة 2: البناء (6-12 شهر)
| # | النشاط | المسؤول | الموعد | المخرج |
|---|--------|---------|--------|--------|
اكتب 4 أنشطة للمرحلة الثانية

### المرحلة 3: التحسين (12-24 شهر)
| # | النشاط | المسؤول | الموعد | المخرج |
|---|--------|---------|--------|--------|
اكتب 4 أنشطة للمرحلة الثالثة
...
|---|--------|-------|---------------|-----------------|---------------|
اكتب 8 مؤشرات أداء لمجال {domain}
ملاحظة: القيمة الحالية = "يُحدد بعد التقييم" لجميع المؤشرات

### أدلة تقييم المؤشرات:
لكل مؤشر، اكتب جدول خطوات التقييم:
...
**درجة الثقة:** تُحدد بعد التقييم الميداني

**تبرير التقييم:**
اكتب فقرة توضح العوامل المؤثرة في تقييم الثقة

### المخاطر الرئيسية:
| # | الخطر | الاحتمالية | الأثر | استراتيجية التخفيف |
|---|-------|-----------|-------|-------------------|
اكتب 4 مخاطر استراتيجية لمجال {domain}

---
**تاريخ الإعداد:** يُحدد لاحقاً

⚠️⚠️⚠️ تذكير نهائي ⚠️⚠️⚠️
1. اكتب محتوى حقيقي ومحدد - لا تكتب قوالب
2. استخدم [SECTION] بين كل قسم
3. جميع الأقسام الستة مطلوبة
4. لا تستخدم أرقام أو نسب محددة مسبقاً"""
        else:
